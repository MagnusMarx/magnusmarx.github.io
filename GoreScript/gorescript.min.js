...
GS.inherit = function(classObj, members) {
    ...
    return Object.getOwnPropertyNames(members).forEach(function(prop) {
        ...
        void 0 !== desc.get ? base.__defineGetter__(prop, desc.get) : base[prop] = members[prop],
        void 0 !== desc.set && base.__defineSetter__(prop, desc.set)
    }),
    base
}
,
...
GS.logOnce = function(id, str) {
    GS.logOnce[id.toString()] !== !0 && (GS.logOnce[id.toString()] = !0,
    console.log(str))
}
,
GS.isFirefox = "undefined" != typeof InstallTrigger,
...
GS.Base.prototype = {
    ...
    load: function() {
        null !== this.requestAnimationFrameId && cancelAnimationFrame(this.requestAnimationFrameId),
    ...
    gameLoop: function() {
        ...
        for (frameTime > .33 && (frameTime = .33),
        this.currentTime = newTime,
        this.accumulator += frameTime; this.accumulator >= this.timeStep; )
            this.update(),
            this.fpsCounter.updates++,
            this.accumulator -= this.timeStep;
        this.draw(),
        this.fpsCounter.frames++,
        newTime - this.fpsCounter.currentTime >= 1e3 && (this.fpsCounter.updateRate = this.fpsCounter.updates,
...
GS.Canvas2d = function(hideOnStart) {
    ...
    this.hideOnStart = void 0 !== hideOnStart && hideOnStart,
    this.minWidth = 1280,
    this.minHeight = 720,
    this.alpha = 1,
    this.redrawOnResize = !0
}
,
GS.Canvas2d.prototype = {
    ...
    _roundedBox: function(p0, p1, radius, filled, color, lineWidth) {
        ...
        var x0 = Math.min(p0.x, p1.x)
          ...
          , y1 = Math.max(p0.y, p1.y);
        this.bufferCtx.save(),
        filled ? this.bufferCtx.fillStyle = color : (this.bufferCtx.strokeStyle = color,
        this.bufferCtx.lineWidth = lineWidth),
        this.bufferCtx.beginPath(),
        ...
        this.bufferCtx.closePath(),
        filled ? this.bufferCtx.fill() : this.bufferCtx.stroke(),
        this.bufferCtx.restore()
    },
    ...
    text: function(pos, str, color, fontSize, textBaseline, textAlign, fontFamily) {
        color = void 0 !== color ? color : "#fff",
        fontSize = void 0 !== fontSize ? fontSize : 16,
    ...
    getTextWidth: function(str, fontSize, fontFamily) {
        ...
        return this.bufferCtx.restore(),
        width
    },
    onResize: function() {
        var canvas;
        this.redrawOnResize && (canvas = this.clone()),
        ...
        this.redrawOnResize && this.screenCtx.drawImage(canvas, 0, 0)
    },
    clone: function() {
...
GS.Detector = {
    run: function(callback) {
        var canvas = document.createElement("canvas");
        if (void 0 === canvas.getContext)
            return void this.displayErrorMessage("HTML5 canvas");
        var webGL = function() {
            try {
                var canvas = document.createElement("canvas");
...
function() {
    ...
    THREE.Vector3.prototype.toString = function(d) {
        return d = d || 0,
        ["[", this.x.toFixed(d), this.y.toFixed(d), this.z.toFixed(d), "]"].join(" ")
    }
    ,
    THREE.Vector3.prototype.equalsEpsilon = function(v, e) {
        return e = e || 1e-4,
        Math.abs(v.x - this.x) < e && Math.abs(v.y - this.y) < e && Math.abs(v.z - this.z) < e
    }
    ,
    ...
    THREE.Box3.prototype.isIntersectionBoxSwept = function() {
        ...
        return function(oldPos, newPos, box, boxPos, point) {
            if (velocity.copy(newPos).sub(oldPos),
            aux1.copy(this.min).add(velocity),
            aux2.copy(this.max).add(velocity),
            auxBox.setFromPoints([this.min, this.max, aux1, aux2]),
            !auxBox.isIntersectionBox(box))
                return !1;
            if (this.isIntersectionBox(box))
                return point.copy(oldPos),
                !0;
            if (auxBox.min.copy(this.min).add(velocity),
            auxBox.max.copy(this.max).add(velocity),
            auxBox.isIntersectionBox(box))
                return point.copy(newPos),
                !0;
            size1.copy(this.max).sub(this.min),
            size2.copy(box.max).sub(box.min),
            velocity.x > 0 ? (invEntry.x = box.min.x - (this.min.x + size1.x),
            invExit.x = box.min.x + size2.x - this.min.x) : (invEntry.x = box.min.x + size2.x - this.min.x,
            invExit.x = box.min.x - (this.min.x + size1.x)),
            velocity.y > 0 ? (invEntry.y = box.min.y - (this.min.y + size1.y),
            invExit.y = box.min.y + size2.y - this.min.y) : (invEntry.y = box.min.y + size2.y - this.min.y,
            invExit.y = box.min.y - (this.min.y + size1.y)),
            velocity.z > 0 ? (invEntry.z = box.min.z - (this.min.z + size1.z),
            invExit.z = box.min.z + size2.z - this.min.z) : (invEntry.z = box.min.z + size2.z - this.min.z,
            invExit.z = box.min.z - (this.min.z + size1.z)),
            0 === velocity.x ? (entry.x = -(1 / 0),
            exit.x = 1 / 0) : (entry.x = invEntry.x / velocity.x,
            exit.x = invExit.x / velocity.x),
            0 === velocity.y ? (entry.y = -(1 / 0),
            exit.y = 1 / 0) : (entry.y = invEntry.y / velocity.y,
            exit.y = invExit.y / velocity.y),
            0 === velocity.z ? (entry.z = -(1 / 0),
            exit.z = 1 / 0) : (entry.z = invEntry.z / velocity.z,
            exit.z = invExit.z / velocity.z);
            var entryTime = entry.maxCoord()
              , exitTime = exit.minCoord();
            return !(entryTime > exitTime || entry.x < 0 && entry.y < 0 && entry.z < 0 || entry.x > 1 || entry.y > 1 || entry.z > 1) && (velocity.multiplyScalar(entryTime),
    ...
    THREE.Ray.prototype.intersectsOrStartsInBox = function(box, optionalTarget) {
        ...
        return box.containsPoint(this.origin) ? result.copy(this.origin) : this.intersectBox(box, result),
        result
    }
    ,
    THREE.Vector2.prototype.toString = function(d) {
        return d = d || 0,
        ["[", this.x.toFixed(d), this.y.toFixed(d), "]"].join(" ")
    }
    ,
    THREE.Vector2.prototype.equalsEpsilon = function(v, e) {
        return e = e || 1e-4,
        Math.abs(v.x - this.x) < e && Math.abs(v.y - this.y) < e
    }
    ,
    ...
    THREE.Box2.prototype.isIntersectionBoxSwept = function() {
        ...
        return function(oldPos, newPos, box, boxPos, point) {
            ...
            var entryTime = entry.maxCoord()
              , exitTime = exit.minCoord();
            return !(entryTime > exitTime || entry.x < 0 && entry.y < 0 || entry.x > 1 || entry.y > 1) && (velocity.multiplyScalar(entryTime),
            point.copy(oldPos).add(velocity),
            !0)
        }
    }(),
    THREE.Box2.prototype.isIntersectionLineSwept = function() {
        ...
        return function(oldPos, newPos, line) {
            ...
            if (velocity.copy(newPos).sub(oldPos),
            boxExtent.copy(this.max).sub(this.min).multiplyScalar(.5),
            boxCenter.copy(this.max).add(this.min).multiplyScalar(.5),
            GS.MathHelper.vec2Normal(line.start, line.end, lineNormal),
            lineNormal.dot(velocity) >= 0)
                return result;
            lineDir.copy(line.end).sub(line.start),
            lineDir.x > 0 ? (lineMin.x = line.start.x,
            lineMax.x = line.end.x) : (lineMin.x = line.end.x,
            lineMax.x = line.start.x),
            lineDir.y > 0 ? (lineMin.y = line.start.y,
            lineMax.y = line.end.y) : (lineMin.y = line.end.y,
            ...
            } else if (lineMin.x > this.max.x || lineMax.x < this.min.x)
                return result;
            if (hitTime > outTime)
                return result;
            if (velocity.y < 0) {
            ...
            } else if (lineMin.y > this.max.y || lineMax.y < this.min.y)
                return result;
            return hitTime > outTime ? result : (result.foundCollision = !0,
    ...
    THREE.BufferGeometry.prototype.fromGeometry = function(geometry, settings) {
        ...
        if (this.addAttribute("normal", new THREE.BufferAttribute(normals,3)),
        vertexColors !== THREE.NoColors) {
        ...
        for (var i = 0, i2 = 0, i3 = 0; i < faces.length; i++,
        i2 += 6,
        i3 += 9) {
            var face = faces[i]
              , a = vertices[face.a]
...
GS.InputHelper = {
    ...
    init: function() {
        ...
        $(document).keydown(function(e) {
            if (that.keyState[e.keyCode] = !0,
            that.ctrl = e.ctrlKey,
            that.alt = e.altKey,
            that.shift = e.shiftKey,
            9 == e.keyCode || 65 == e.keyCode && e.ctrlKey || 90 == e.keyCode && e.ctrlKey || 71 == e.keyCode && e.ctrlKey || 189 == e.keyCode && e.ctrlKey || 187 == e.keyCode && e.ctrlKey || 48 == e.keyCode && e.ctrlKey)
        ...
        $(document).mousedown(function(e) {
            1 == e.which && (that.leftMouseDown = !0),
            2 == e.which && (that.middleMouseDown = !0),
            3 == e.which && (that.rightMouseDown = !0)
        }),
        $(document).mouseup(function(e) {
            1 == e.which && (that.leftMouseDown = !1),
            2 == e.which && (that.middleMouseDown = !1),
            3 == e.which && (that.rightMouseDown = !1)
        }),
    ...
    isKeyDown: function(keyCode, dontModifyPressed) {
        return dontModifyPressed || this.keyState[keyCode] && (this.pressedKeys.indexOf(keyCode) == -1 && this.pressedKeys.push(keyCode),
        this.keysPressed = !0),
...
GS.LineHelper = {
    ...
    intersectionLineSegments: function() {
        ...
        return function(s1, s2, p) {
            return p = p || p0,
            this.intersectionLines(s1, s2, p, !0)
        }
    }(),
    intersectionLines: function(s1, s2, p, compareLineSegments) {
        ...
        return (!compareLineSegments || ua >= 0 && ua <= 1 && ub >= 0 && ub <= 1) && (p.x = x1 + ua * (x2 - x1),
        p.y = y1 + ua * (y2 - y1),
        !0)
    },
    intersectionLineSegmentBox: function() {
        ...
        return function(seg, box) {
            return box0.setFromPoints([seg.start, seg.end]),
            !!box.isIntersectionBox(box0) && (!(!box.containsPoint(seg.start) || !box.containsPoint(seg.end)) || (seg0.start.x = box.min.x,
            seg0.start.y = box.min.y,
            seg0.end.x = box.max.x,
            seg0.end.y = box.min.y,
            !!this.intersectionLineSegments(seg, seg0, p) || (seg0.start.x = box.max.x,
            seg0.start.y = box.min.y,
            seg0.end.x = box.max.x,
            seg0.end.y = box.max.y,
            !!this.intersectionLineSegments(seg, seg0, p) || (seg0.start.x = box.max.x,
            seg0.start.y = box.max.y,
            seg0.end.x = box.min.x,
            seg0.end.y = box.max.y,
            !!this.intersectionLineSegments(seg, seg0, p) || (seg0.start.x = box.min.x,
            seg0.start.y = box.max.y,
            seg0.end.x = box.min.x,
            seg0.end.y = box.min.y,
            !!this.intersectionLineSegments(seg, seg0, p))))))
        }
    }(),
...
GS.MathHelper = {
    ...
    getConvexHullPoints: function(P) {
        ...
        for (var i = 0; i < n; i++) {
            for (; k >= 2 && cross(H[k - 2], H[k - 1], P[i]) <= 0; )
                k--;
            H[k++] = P[i]
        }
        for (var t = k + 1, i = n - 2; i >= 0; i--) {
            for (; k >= t && cross(H[k - 2], H[k - 1], P[i]) <= 0; )
                k--;
            H[k++] = P[i]
        }
        return H.length = k - 1,
    ...
    vec2Normal: function(v0, v1, optionalTarget) {
        var result = optionalTarget || new THREE.Vector2
          , dx = v1.x - v0.x
          , dy = v1.y - v0.y;
        return result.set(dy, -dx).normalize(),
        result
    },
...
GS.SmoothNumber.prototype = {
    ...
    update: function() {
        this.value < this.targetValue && (this.value += this.speed,
        this.value > this.targetValue && (this.value = this.targetValue)),
        this.value > this.targetValue && (this.value -= this.speed,
        this.value < this.targetValue && (this.value = this.targetValue))
    }
},
GS.SmoothVector3 = function(x, y, z, speed) {
    ...
    this.speed = void 0 !== speed ? speed : .1,
...
GS.SmoothVector3.prototype = GS.inherit(THREE.Vector3, {
    ...
    updateProperty: function(name) {
        this[name] < this.targetValue[name] && (this[name] += this.speed,
        this[name] > this.targetValue[name] && (this[name] = this.targetValue[name])),
        this[name] > this.targetValue[name] && (this[name] -= this.speed,
        this[name] < this.targetValue[name] && (this[name] = this.targetValue[name]))
    }
}),
GS.PolygonHelper = {
    ...
    getTriangulatedPolygonsFromLineSegments: function(segments, gridCellSize) {
        for (var result = this.getPolygonsFromLineSegments(segments, gridCellSize), i = 0; i < result.polygons.length; i++) {
            ...
            this.areVerticesClockwise(vertices) && this.reverseVertices(vertices),
            result.polygons[i] = {
    ...
    sectorContainsPoint: function(sector, p, useCollisionVertices) {
        var i, v = useCollisionVertices ? sector.collisionVertices : sector.vertices, n = v.length, c = !1, j = 0;
        for (i = 0,
        j = n - 1; i < n; j = i++) {
            var start = v[i]
              , end = v[j];
    ...
    intersectionSectorBox: function() {
        var box0 = new THREE.Box2
          ...
          , boxPoints = (new THREE.Vector2,
        [new THREE.Vector2, new THREE.Vector2, new THREE.Vector2, new THREE.Vector2]);
        return function(sector, box) {
            if (box0.setFromPoints(sector.vertices),
            !box.isIntersectionBox(box0))
                return !1;
            if (box.containsBox(box0))
                return !0;
            boxPoints[0].x = box.min.x,
            boxPoints[0].y = box.min.y,
            boxPoints[1].x = box.max.x,
            boxPoints[1].y = box.min.y,
            boxPoints[2].x = box.max.x,
            boxPoints[2].y = box.max.y,
            boxPoints[3].x = box.min.x,
            boxPoints[3].y = box.max.y;
            ...
            for (var i = 0; i < sector.vertices.length - 1; i++)
                if (seg0.start.copy(sector.vertices[i]),
                seg0.end.copy(sector.vertices[i + 1]),
                GS.LineHelper.intersectionLineSegmentBox(seg0, box))
                    return !0;
            return seg0.start.copy(sector.vertices[sector.vertices.length - 1]),
    ...
    retriangulateSectors: function(sectors) {
        function sectorEdgeContainsVertex(a, v) {
            ...
            for (i = 0; i < a.vertices.length - 1; i++) {
                ...
                offset != -1)
                    return result.edgeIndex = i,
                    result.offset = offset,
                    result
            }
            if (!a.vertices[i].equalsEpsilon(v) && !a.vertices[0].equalsEpsilon(v))
                return offset = GS.LineHelper.pointOffsetOnLineSegment({
                    start: a.vertices[i],
                    end: a.vertices[0]
                }, v),
                offset != -1 ? (result.edgeIndex = i,
                result.offset = offset,
                result) : void 0
        }
        ...
        for (var sectorEdges = [], i = 0; i < sectors.length; i++) {
            for (var edges = [], j = 0; j < sectors[i].vertices.length; j++)
    ...
    retriangulateSegments: function(segments) {
        ...
        function addVertices(seg, endPoint, compare) {
            for (var points = new GS.Set, i = 0; i < segments.length; i++) {
                var s = segments[i];
                s !== seg && (s.start.equalsEpsilon(endPoint) || s.end.equalsEpsilon(endPoint)) && (s.topY < seg.topY && s.topY > seg.bottomY && points.add(s.topY),
                s.bottomY < seg.topY && s.bottomY > seg.bottomY && points.add(s.bottomY))
            }
        ...
        for (var i = 0; i < segments.length; i++) {
            ...
            if (seg.type !== GS.SegmentTypes.TVScreen && seg.type !== GS.SegmentTypes.Switch) {
                seg.indices = [0, 1, 2],
                seg.vertices = [new THREE.Vector3(seg.start.x,seg.topY,seg.start.y), new THREE.Vector3(seg.end.x,seg.topY,seg.end.y)];
                ...
                if (n = addVertices(seg, seg.start, leftSideCompare),
                n > 0)
                    for (var j = 0; j < n; j++)
                        seg.indices.push(m),
    ...
    detriangulateSectors: function(sectors) {
        for (var i = 0; i < sectors.length; i++) {
            for (var sector = sectors[i], points = [sector.vertices[0]], n = sector.vertices.length, j = 1; j < n; j++)
                j < n - 1 ? GS.LineHelper.arePointsCollinear(sector.vertices[j - 1], sector.vertices[j], sector.vertices[j + 1]) || points.push(sector.vertices[j]) : points.push(sector.vertices[j]);
    ...
    getSectorCentroid: function(sector) {
        function getPoint(n) {
            return n === sector.vertices.length && (n = 0),
            sector.vertices[n]
        }
        for (var p1, p2, f, twicearea = 0, x = 0, y = 0, nPts = sector.vertices.length, i = 0, j = nPts - 1; i < nPts; j = i++)
            p1 = getPoint(i),
            p2 = getPoint(j),
            ...
            x += (p1.x + p2.x) * f,
            y += (p1.y + p2.y) * f;
        return f = 3 * twicearea,
        new THREE.Vector2(x / f,y / f)
    }
},
...
GS.Graph.prototype = {
    ...
    removeEdge: function(start, end) {
        ...
        if (!this.equalsFunc(start, end))
            ...
            }, i = this.edges.length - 1; i >= 0; i--)
                equals(edge, this.edges[i]) && this.edges.splice(i, 1)
    },
    addVertex: function(vertex) {
        var result = this.vertexExists(vertex);
        return result > -1 ? result : (this.vertices.push(vertex),
    ...
    initNextArray: function() {
        for (var i = 0; i < this.vertices.length; i++)
            for (var j = 0; j < this.vertices.length; j++)
                i == j || this.dist[i][j] == 1 / 0 ? this.next[i][j] = 0 : this.next[i][j] = i
    },
    getPath: function(i, j) {
        if (i == j)
            return [];
        var path = this.constructPath(i, j);
        return void 0 !== path && (path.unshift(i),
        path.push(j)),
        path
    },
...
GS.PlanarGraph.prototype = GS.inherit(GS.Graph, {
    computeMinimumCycleBasis: function() {
        ...
        this.cycles.length > 100 ? (console.log("cycle count > 100; using horton"),
        this.selectCyclesHorton()) : this.selectCyclesNonOverlapping()
    },
    computeAllCycles: function() {
        ...
        function fromPathSet(set) {
            for (var a = new GS.Set((void 0),equals), e = set.elements, n = e.length, i = 0; i < n - 1; i++)
                a.add({
                    start: e[i],
                    end: e[i + 1]
                });
        ...
        for (var that = this, cycles = [], path0 = new GS.Set, path1 = new GS.Set, i = 0; i < this.vertices.length; i++)
            for (var j = 0; j < this.edges.length; j++) {
                ...
                if (1 == it.elements.length && it.elements[0] == i) {
                    ...
                    cycle.add(edge),
                    inCycles(cycle) || cycles.push(cycle)
                }
            }
    ...
    pointInCycle: function(p, cycle) {
        var i, v = this.vertices, idx = cycle.elements, n = idx.length - 1, c = !1, j = 0;
        for (i = 0,
        j = n - 1; i < n; j = i++) {
    ...
    cyclesOverlap: function(a, b) {
        for (var center = new THREE.Vector2, edgesA = a.difference(b).elements, v = this.vertices, i = 0; i < edgesA.length; i++) {
            var edge = edgesA[i];
            if (center.x = (v[edge.start].x + v[edge.end].x) / 2,
            center.y = (v[edge.start].y + v[edge.end].y) / 2,
            this.pointInCycle(center, b))
                return !0
        }
        for (var edgesB = b.difference(a).elements, i = 0; i < edgesB.length; i++) {
            ...
            if (center.x = (v[edge.start].x + v[edge.end].x) / 2,
            center.y = (v[edge.start].y + v[edge.end].y) / 2,
            this.pointInCycle(center, a))
                return !0
        }
        return !1
    },
    selectCyclesNonOverlapping: function() {
        for (var n = this.cycles.length, graph = new GS.Graph, i = 0; i < n; i++)
            graph.addVertex(i);
        for (var i = 0; i < n - 1; i++)
            for (var j = i + 1; j < n; j++)
                graph.addEdge(i, j);
        for (var i = 0; i < n - 1; i++)
            for (var j = i + 1; j < n; j++)
                this.cyclesOverlap(this.cycles[i], this.cycles[j]) && graph.removeEdge(i, j);
        graph.computeMaximalCliques(),
        graph.maximalCliques.length > 0 && (this.minimumCycleBasis = graph.maximalCliques[0].elements)
    },
    selectCyclesHorton: function() {
        for (var m = this.cycles.length, n = this.edges.length, matrix = new GS.Matrix(m,n), i = 0; i < m; i++)
            for (var j = 0; j < n; j++)
                this.cycles[i].elemExists(this.edges[j]) && (matrix.matrix[i][j] = 1);
...
GS.Set.prototype = {
    add: function(elem) {
        return this.elemExists(elem) || this.elements.push(elem),
        this
    },
    ...
    intersection: function(set) {
        for (var elements = set.elements, it = [], i = 0; i < this.elements.length; i++)
            for (var j = 0; j < elements.length; j++)
                this.equalsFunc(this.elements[i], elements[j]) && it.push(this.elements[i]);
        return new GS.Set(it,this.equalsFunc)
    },
    union: function(set) {
        for (var un = new GS.Set(this.elements.slice(),this.equalsFunc), elements = set.elements, i = 0; i < elements.length; i++)
            un.add(elements[i]);
        return un
    },
    difference: function(set) {
        for (var diff = [], i = 0; i < this.elements.length; i++)
            set.elemExists(this.elements[i]) || diff.push(this.elements[i]);
        ...
        for (var that = this, addRow = function(r0, r1) {
            ...
            if (found)
                for (var k = 0; k < this.rowCount; k++)
                    k != i && 1 == this.matrix[k][j] && addRow(k, i)
        }
    },
...
GS.Rebound.prototype = {
    ...
    modifyKeybind: function(keybind) {
        var that = this;
        this.modifyingKeybind || (this.modifyingKeybind = !0,
        this.onModifyingKeybindStart({
            keybind: keybind
        }),
        $(document).on("keydown.modifyKeybind", function(e) {
            ...
            that.modifyingKeybind = !1,
            that.onModifyingKeybindStop({
                keybind: keybind,
                success: ok
            }),
            e.preventDefault()
    ...
    changeKey: function(keybind, e) {
        ...
        return existing && (existing.code = void 0,
        existing.mouse = void 0,
        existing.button = void 0,
        existing.controlName = "NOT BOUND"),
        keybind.code = code,
        keybind.mouse = void 0,
        keybind.button = void 0,
        keybind.controlName = name,
        !0
    },
    changeMouse: function(keybind, e) {
        var name;
        if (1 === e.which ? name = "MOUSE LEFT" : 2 === e.which && (name = "MOUSE MIDDLE"),
        !name)
            return !1;
...
GS.DebugUI = {
    ...
    set visible(value) {
        this._visible = value,
        !value && this.ctx && this.ctx.clearRect(0, 0, this.width, this.height)
    },
    get visible() {
    ...
    update: function() {
        for (var i = this.tempLines.length - 1; i >= 0; i--) {
            ...
            line.timeout--,
            0 === line.timeout && (this.tempLines.splice(i, 1),
            this.hasChanged = !0)
        }
        this.hasChanged && (this.calculateOverlayCoords(),
        this.draw(),
        this.hasChanged = !1)
    },
    draw: function() {
        if (this._visible && (this.ctx.clearRect(0, 0, this.width, this.height),
        this.visibleStaticLines > 0 || this.tempLines.length > 0)) {
            this.ctx.save(),
            ...
            for (var i in this.staticLines)
                this.staticLines[i].visible && (this.ctx.fillText(this.staticLines[i].text, this.overlayX + this.overlayPadding, y),
    ...
    calculateOverlayCoords: function() {
        ...
        for (var i in this.staticLines)
            this.staticLines[i].visible && this.visibleStaticLines++;
        ...
        for (var i in this.staticLines)
            this.staticLines[i].visible && (this.overlayWidth = Math.max(this.overlayWidth, this.ctx.measureText(this.staticLines[i].text).width + 2 * this.overlayPadding));
        for (var i = 0; i < this.tempLines.length; i++)
            this.overlayWidth = Math.max(this.overlayWidth, this.ctx.measureText(this.tempLines[i].text).width + 2 * this.overlayPadding)
    },
    trackNumericValue: function(id, numericValue) {
        void 0 === this.valueTracking[id] && (this.valueTracking[id] = {},
        this.valueTracking[id].min = 1 / 0,
        this.valueTracking[id].max = -(1 / 0),
    ...
    setStaticLine: function(id, text) {
        text = id + ": " + text,
        id in this.staticLines ? this.staticLines[id].text = text : this.staticLines[id] = {
            text: text,
            visible: !0
        },
        this.hasChanged = !0
    ...
    addTempLine: function(text, timeout) {
        this.tempLines.length == this.maxTempLines && this.tempLines.shift(),
        timeout = void 0 !== timeout ? timeout : this.defaultTimeout;
        var line = {
            text: text,
            timeout: timeout
        };
...
GS.LoadingUI.prototype = {
    ...
    draw: function() {
        !this.hidden && this.redraw && (this.redraw = !1,
        this.cvs.clear(),
        this.spinnerOnly ? this.cvs.text(this.screenInfo.center, "Imagine it's 1994...", "#fff", 40, "middle", "center", GS.UIFont) : (this.cvs.box(this.loadingBox.topLeft, this.loadingBox.bottomRight, "#fff", 3),
    ...
    updateLoadingBoxContent: function() {
        ...
        this.loadingBox.contentCurrentBottomRight.x = this.loadingBox.contentTopLeft.x + Math.floor(width * this.percentLoaded * .01),
        this.redraw = !0
    },
    show: function() {
        this.cvs.show(),
    ...
    onResize: function() {
        ...
        this.loadingBox.topLeft.x -= Math.floor(.5 * this.loadingBox.width * this.screenInfo.width),
        this.loadingBox.topLeft.y -= Math.floor(.5 * this.loadingBox.height * this.screenInfo.height),
        this.loadingBox.bottomRight.copy(this.screenInfo.center),
        this.loadingBox.bottomRight.x += Math.floor(.5 * this.loadingBox.width * this.screenInfo.width),
        this.loadingBox.bottomRight.y += Math.floor(.5 * this.loadingBox.height * this.screenInfo.height),
...
GS.UIManager.prototype = {
    ...
    initComponents: function(assets, grid) {
        ...
        this.components = [],
        void 0 !== this.grid && (this.powerBars = new GS.UIComponents.PowerBars(this.vectorCanvas,this.assets,this.grid.player),
        this.powerBars.init(),
        ...
        void 0 === this.menu && (this.menu = new GS.UIComponents.Menu(this.vectorCanvas,this.assets),
        this.menu.init()),
    ...
    draw: function() {
        if (this.menuActive)
            this.cvs.clear(),
            this.menuOnly && this.vectorCanvas.drawImage(this.menuBackOffset, this.menuBackPos, this.assets.images.menu_back, this.menuBackSize, !1),
            this.menu.draw(),
            this.cvs.flip();
        else if (this.winScreen.visible)
            this.cvs.clear(),
            this.winScreen.draw(),
            this.cvs.flip();
        else if (this.automap.visible)
            (this.overrideRedraw || this.automap.needsRedraw) && (this.overrideRedraw && (this.overrideRedraw = !1),
            this.cvs.clear(),
            this.automap.draw(),
            this.automap.needsRedraw = !1,
            this.cvs.flip());
        else if (!this.hidden && this.checkIfRedraw()) {
            if (this.cvs.clear(),
            this.showHUD)
                for (var i = 0; i < this.components.length; i++)
                    this.components[i].visible && (this.components[i].draw(),
                    this.components[i].needsRedraw = !1);
            this.cvs.flip()
        }
    },
...
GS.VectorCanvas.prototype = {
    ...
    convertToScreenCoords: function(v0, v, fixedOffset) {
        ...
        void 0 !== fixedOffset && (v.x += fixedOffset.x,
        v.y += fixedOffset.y)
    },
    fixedLine: function() {
    ...
    line: function() {
        ...
        return function(fixedOffset, pos, lineOffset, isFixedLineOffset, color, lineWidth) {
            ...
            isFixedLineOffset ? v1.copy(v0).add(lineOffset) : (this.convertToScreenCoords(lineOffset, v1),
            v1.add(v0)),
            this.cvs.line(v0, v1, color, lineWidth)
        }
    }(),
    box: function() {
        ...
        return function(fixedOffset, pos, size, isFixedSize, color, lineWidth) {
            this.convertToScreenCoords(pos, v0, fixedOffset),
            isFixedSize ? v1.copy(v0).add(size) : (this.convertToScreenCoords(size, v1),
            v1.add(v0)),
    ...
    drawImage: function() {
        ...
        return function(fixedOffset, pos, img, size, isFixedSize) {
            this.convertToScreenCoords(pos, v0, fixedOffset),
            isFixedSize ? v1.copy(size) : this.convertToScreenCoords(size, v1),
...
GS.UIComponents.Automap.prototype = {
    ...
    update: function() {
        ...
        for (regionsDiscovered > this.oldRegionsDiscovered && (this.needsRedraw = !0,
        this.oldRegionsDiscovered = regionsDiscovered),
        this.player.position.toVector2(this.currentPlayerPosition),
        this.currentPlayerPosition.equalsEpsilon(this.oldPlayerPosition) || (this.needsRedraw = !0,
        this.oldPlayerPosition.copy(this.currentPlayerPosition)),
        this.player.direction.toVector2(this.currentPlayerDirection),
        this.currentPlayerDirection.equalsEpsilon(this.oldPlayerDirection) || (this.needsRedraw = !0,
        this.oldPlayerDirection.copy(this.currentPlayerDirection)); GS.InputHelper.mouseWheelEvents.length > 0; ) {
            var delta = GS.InputHelper.mouseWheelEvents.shift();
            delta < 0 && (this.zoom -= this.zoomSpeed),
            delta > 0 && (this.zoom += this.zoomSpeed),
    ...
    drawPlayer: function() {
        ...
        return function() {
            ...
            p0.x = size * Math.sin(Math.PI / 180 * (450 - xAngle)),
            p0.y = size * Math.cos(Math.PI / 180 * (450 - xAngle)),
            p1.x = size * Math.sin(Math.PI / 180 * (300 - xAngle)),
            p1.y = size * Math.cos(Math.PI / 180 * (300 - xAngle)),
            p2.x = size * Math.sin(Math.PI / 180 * (240 - xAngle)),
            p2.y = size * Math.cos(Math.PI / 180 * (240 - xAngle)),
    ...
    drawRegion: function() {
        ...
        return function(region) {
            function drawSectorLine(sector, x0, x1, color, lineWidth) {
                ...
                p1.sub(p0),
                that.cvs.line(p0, that.mapDraw.pos, p1, !0, color, lineWidth)
            }
            var that = this
...
GS.UIComponents.Crosshair = function(vectorCanvas, assets, player) {
    ...
    this.size = 10,
    this.lineWidth = 3,
    this.pos = new THREE.Vector2(.5,.5),
    this.hOffset = new THREE.Vector2((-this.size),0),
    this.hLineOffset = new THREE.Vector2(2 * this.size,0),
    this.vOffset = new THREE.Vector2(0,(-this.size)),
    this.vLineOffset = new THREE.Vector2(0,2 * this.size),
    this.visible = !0
}
,
GS.UIComponents.Crosshair.prototype = {
    ...
    update: function() {},
    draw: function() {
        this.cvs.line(this.hOffset, this.pos, this.hLineOffset, !0, GS.UIColors.foreground, this.lineWidth),
        this.cvs.line(this.vOffset, this.pos, this.vLineOffset, !0, GS.UIColors.foreground, this.lineWidth)
    }
},
...
GS.UIComponents.Menu.prototype = {
    ...
    initTopPanel: function() {
        var that = this;
        this.topPanel = new GS.UIComponents.MenuPanel(this.cvs,new THREE.Vector2((-400),(-160)),new THREE.Vector2(.5,.5),new THREE.Vector2(800,520),72,80),
        this.btnNewGame = this.topPanel.addButton("New Game"),
        this.btnNewGame.onClick = function() {
            GAME.newGame()
        }
        ,
        this.btnLevelSelect = this.topPanel.addButton("Level Select"),
        this.btnLevelSelect.onClick = function() {
    ...
    initOptionsPanel: function() {
        var that = this;
        this.optionsPanel = new GS.UIComponents.MenuPanel(this.cvs,new THREE.Vector2((-400),(-160)),new THREE.Vector2(.5,.5),new THREE.Vector2(800,520),60,65),
        this.btnGraphics = this.optionsPanel.addButton("Graphics"),
        this.btnGraphics.onClick = function() {
        ...
        this.btnGameplay.onClick = function() {
            that.activePanel = that.gameplayPanel
        }
        ,
        this.btnControls = this.optionsPanel.addButton("Controls"),
        this.btnControls.onClick = function() {
            that.activePanel = that.controlsPanel
        }
        ,
        this.btnCheats = this.optionsPanel.addButton("Cheats"),
        this.btnCheats.onClick = function() {
            that.activePanel = that.cheatsPanel
        }
        ,
        this.optionsPanel.addEmptyRow(),
        this.btnOptionsBack = this.optionsPanel.addButton("Back"),
        this.btnOptionsBack.onClick = function() {
            that.activePanel = that.topPanel
        }
    },
    initGraphicsPanel: function() {
        var that = this;
        this.graphicsPanel = new GS.UIComponents.MenuPanel(this.cvs,new THREE.Vector2((-400),(-160)),new THREE.Vector2(.5,.5),new THREE.Vector2(800,520),40,43),
        this.btnToggleHalfSize = this.graphicsPanel.addToggleButton("half-size rendering"),
        this.btnToggleHalfSize.button.currentStateIndex = GS.Settings.halfSize === !0 ? 0 : 1,
        this.btnToggleHalfSize.button.onClick = function(e) {
            GS.Settings.halfSize = "on" === e.state
        }
        ,
        this.graphicsPanel.addEmptyRow(),
        this.btnToggleSSAO = this.graphicsPanel.addToggleButton("SSAO"),
        this.btnToggleSSAO.button.currentStateIndex = GS.Settings.ssao === !0 ? 0 : 1,
        this.btnToggleSSAO.button.onClick = function(e) {
            GS.Settings.ssao = "on" === e.state
        }
        ,
        this.btnToggleBloom = this.graphicsPanel.addToggleButton("Bloom"),
        this.btnToggleBloom.button.currentStateIndex = GS.Settings.bloom === !0 ? 0 : 1,
        this.btnToggleBloom.button.onClick = function(e) {
            GS.Settings.bloom = "on" === e.state
        }
        ,
        this.btnToggleNoise = this.graphicsPanel.addToggleButton("Noise Filter"),
        this.btnToggleNoise.button.currentStateIndex = GS.Settings.noise === !0 ? 0 : 1,
        this.btnToggleNoise.button.onClick = function(e) {
            GS.Settings.noise = "on" === e.state
        }
        ,
        this.btnToggleVignette = this.graphicsPanel.addToggleButton("Vignette"),
        this.btnToggleVignette.button.currentStateIndex = GS.Settings.vignette === !0 ? 0 : 1,
        this.btnToggleVignette.button.onClick = function(e) {
            GS.Settings.vignette = "on" === e.state
        }
        ,
        this.btnToggleFXAA = this.graphicsPanel.addToggleButton("FXAA"),
        this.btnToggleFXAA.button.currentStateIndex = GS.Settings.fxaa === !0 ? 0 : 1,
        this.btnToggleFXAA.button.onClick = function(e) {
            GS.Settings.fxaa = "on" === e.state
        }
        ,
        this.graphicsPanel.addEmptyRow(),
        this.numberPickerFOV = this.graphicsPanel.addNumberPicker("FOV", GS.Settings.fov, GS.Settings.fovMin, GS.Settings.fovMax, 5),
        this.numberPickerFOV.numberPicker.onChange = function(e) {
            GS.Settings.fov = e.value
        }
        ,
        this.btnToggleShowFPS = this.graphicsPanel.addToggleButton("Show FPS"),
        this.btnToggleShowFPS.button.currentStateIndex = GS.Settings.showFPS === !0 ? 0 : 1,
        this.btnToggleShowFPS.button.onClick = function(e) {
            GS.Settings.showFPS = "on" === e.state
        }
        ,
        this.graphicsPanel.addEmptyRow(),
        this.btnGraphicsBack = this.graphicsPanel.addButton("Back"),
        this.btnGraphicsBack.onClick = function() {
            that.activePanel = that.optionsPanel
        }
    },
    initSoundPanel: function() {
        var that = this;
        this.soundPanel = new GS.UIComponents.MenuPanel(this.cvs,new THREE.Vector2((-400),(-160)),new THREE.Vector2(.5,.5),new THREE.Vector2(800,520),40,43),
        this.numberPickerSound = this.soundPanel.addNumberPicker("SFX Volume", GS.Settings.sound, GS.Settings.soundMin, GS.Settings.soundMax, 1),
        this.numberPickerSound.numberPicker.onChange = function(e) {
            GS.Settings.sound = e.value
        }
        ,
        this.numberPickerMusic = this.soundPanel.addNumberPicker("Music Molume", GS.Settings.music, GS.Settings.musicMin, GS.Settings.musicMax, 1),
        this.numberPickerMusic.numberPicker.onChange = function(e) {
            GS.Settings.music = e.value
        }
        ,
        this.soundPanel.addEmptyRow(),
        this.btnSoundBack = this.soundPanel.addButton("Back"),
        this.btnSoundBack.onClick = function() {
            that.activePanel = that.optionsPanel
        }
    },
    initGameplayPanel: function() {
        var that = this;
        this.gameplayPanel = new GS.UIComponents.MenuPanel(this.cvs,new THREE.Vector2((-400),(-160)),new THREE.Vector2(.5,.5),new THREE.Vector2(800,520),40,43),
        this.btnToggleViewBob = this.gameplayPanel.addToggleButton("View Bobbing"),
        this.btnToggleViewBob.button.currentStateIndex = GS.Settings.viewBob === !0 ? 0 : 1,
        this.btnToggleViewBob.button.onClick = function(e) {
            GS.Settings.viewBob = "on" === e.state
        }
        ,
        this.btnToggleWeaponBob = this.gameplayPanel.addToggleButton("Weapon Bobbing"),
        this.btnToggleWeaponBob.button.currentStateIndex = GS.Settings.weaponBob === !0 ? 0 : 1,
        this.btnToggleWeaponBob.button.onClick = function(e) {
        ...
        this.btnToggleShowHUD.button.currentStateIndex = GS.Settings.showHUD === !0 ? 0 : 1,
        this.btnToggleShowHUD.button.onClick = function(e) {
            GS.Settings.showHUD = "on" === e.state
        }
        ,
        this.btnToggleShowWeapon = this.gameplayPanel.addToggleButton("Show Weapon"),
        this.btnToggleShowWeapon.button.currentStateIndex = GS.Settings.showWeapon === !0 ? 0 : 1,
        this.btnToggleShowWeapon.button.onClick = function(e) {
            GS.Settings.showWeapon = "on" === e.state
        }
        ,
        this.gameplayPanel.addEmptyRow(),
        this.btnGameplayBack = this.gameplayPanel.addButton("Back"),
        this.btnGameplayBack.onClick = function() {
            that.activePanel = that.optionsPanel
        }
    },
    initCheatsPanel: function() {
        var that = this;
        this.cheatsPanel = new GS.UIComponents.MenuPanel(this.cvs,new THREE.Vector2((-400),(-160)),new THREE.Vector2(.5,.5),new THREE.Vector2(800,520),40,43),
        ...
        ,
        this.btnToggleFly = this.cheatsPanel.addToggleButton("Fly Mod"),
        ...
        ,
        this.btnToggleNoclip = this.cheatsPanel.addToggleButton("No-Clip Mod"),
        ...
        this.btnCheatsGiveAll.onClick = function() {
            ...
            GS.DebugUI.addTempLine("Max Ammo and Weapons Given")
        }
        ,
        this.cheatsPanel.addEmptyRow(),
        this.btnCheatsBack = this.cheatsPanel.addButton("Back"),
        this.btnCheatsBack.onClick = function() {
            that.activePanel = that.optionsPanel
        }
    },
    initLevelSelectPanel: function() {
        var that = this;
        this.levelSelectPanel = new GS.UIComponents.MenuPanel(this.cvs,new THREE.Vector2((-400),(-160)),new THREE.Vector2(.5,.5),new THREE.Vector2(800,520),60,140),
        this.btnLevel1 = this.levelSelectPanel.addImageButton("airstrip one", this.assets.images.thumb_airstrip1),
        this.btnLevel1.onClick = function() {
        ...
        this.btnLevel2.onClick = function() {
        ...
        this.btnLevelSelectBack.onClick = function() {
            that.activePanel = that.topPanel
        }
    },
    initControlsPanel: function() {
        var that = this;
        this.controlsPanel = new GS.UIComponents.MenuPanel(this.cvs,new THREE.Vector2((-400),(-160)),new THREE.Vector2(.5,.5),new THREE.Vector2(800,520),28,31),
        this.btnToggleMouseInvertY = this.controlsPanel.addToggleButton("invert mouse y axis"),
        this.btnToggleMouseInvertY.button.currentStateIndex = GS.Settings.mouseInvertY === !0 ? 0 : 1,
        this.btnToggleMouseInvertY.button.onClick = function(e) {
            GS.Settings.mouseInvertY = "on" === e.state
        }
        ,
        this.numberPickerMouse = this.controlsPanel.addNumberPicker("mouse sensitivity", GS.Settings.mouse, GS.Settings.mouseMin, GS.Settings.mouseMax, 1),
        this.numberPickerMouse.numberPicker.onChange = function(e) {
        ...
        this.btnToggleAutomap.button.disabled = !0,
        ...
        this.btnControlsBack.onClick = function() {
            that.activePanel = that.optionsPanel
        }
        ,
    ...
    attachReboundEventHandlers: function() {
        ...
        GS.KeybindSettings.rebound.onModifyingKeybindStop = function(e) {
            ...
            if (e.success) {
                ...
                notifications && (notifications.useText = "[" + GS.Keybinds.use.controlName + "] to use",
    ...
    initCreditsPanel: function() {
        var that = this;
        this.creditsPanel = new GS.UIComponents.MenuPanel(this.cvs,new THREE.Vector2((-400),(-160)),new THREE.Vector2(.5,.5),new THREE.Vector2(800,520),30,33),
        ...
        this.btnCreditsBack.onClick = function() {
            that.activePanel = that.topPanel
        }
    },
    initFooter: function() {
        this.label1 = new GS.UIComponents.MenuLabel(this.cvs,GS.GameVersion,new THREE.Vector2((-20),(-70)),new THREE.Vector2(1,1)),
        this.label1.textAlign = "right",
        this.label1.fontSize = 30,
        this.children.push(this.label1),
        this.label2 = new GS.UIComponents.MenuLabel(this.cvs,GS.ReleaseDate,new THREE.Vector2((-20),(-35)),new THREE.Vector2(1,1)),
    ...
    draw: function() {
        this.ingame && this.cvs.boxFill(this.background.offset, this.background.pos, this.background.size, !1, this.backgroundColor),
        this.cvs.drawImage(this.logo.offset, this.logo.pos, this.logo.image, this.logo.size, !0),
...
GS.UIComponents.MenuButton = function(vectorCanvas, text, offset, pos, size, onClick, states) {
    ...
    this.textOffset = this.offset.clone().add(this.size.clone().multiplyScalar(.5)),
    this.hover = !1,
    this.active = !1,
    this.disabled = !1,
...
GS.UIComponents.MenuButton.prototype = {
    ...
    update: function() {
        ...
        this.disabled ? this.foregroundColor = GS.UIColors.foregroundDisabled : (this.foregroundColor = GS.UIColors.foreground,
        mx >= this.min.x && my >= this.min.y && mx < this.max.x && my < this.max.y ? (this.hover = !0,
        this.backgroundColor = GS.UIColors.buttonHover) : this.hover = !1,
        this.hover && (GS.InputHelper.leftMouseDown ? (this.active = !0,
        this.backgroundColor = GS.UIColors.buttonActive) : this.active && (void 0 !== this.states ? (this._currentStateIndex++,
        this._currentStateIndex >= this.states.length && (this._currentStateIndex = 0),
    ...
    draw: function() {
        !this.disabled && this.hover && this.cvs.roundedBoxFill(this.offset, this.pos, this.size, !0, this.borderRadius, this.backgroundColor),
    ...
    set currentStateIndex(value) {
        void 0 !== this.states && value >= 0 && value < this.states.length && (this._currentStateIndex = value,
        this.text = this.states[this._currentStateIndex])
    },
...
GS.UIComponents.MenuImageButton.prototype = {
    ...
    init: function() {
        ...
        button.textOffset.x -= .1 * this.size.x,
        button.textAlign = "left",
        button.fontSize = this.fontSize,
        this.button = button,
        this.imageSize = new THREE.Vector2(231,130),
        this.imageOffset = this.offset.clone(),
        this.imageOffset.x += .2 * this.size.x - this.imageSize.x / 2,
        this.imageOffset.y += .5 * this.size.y - this.imageSize.y / 2
    },
    update: function() {
        this.button.update()
    },
    draw: function() {
        this.button.draw(),
        this.cvs.drawImage(this.imageOffset, this.pos, this.image, this.imageSize, !0)
    }
},
...
GS.UIComponents.MenuNumberPicker = function(vectorCanvas, offset, pos, size, fontSize, value, min, max, step, onChange) {
    ...
    this.pos = pos,
    this.size = size,
    this.value = void 0 !== value ? value : 5,
    this.min = void 0 !== min ? min : 1,
    this.max = void 0 !== max ? max : 10,
    this.step = void 0 !== step ? step : 1,
...
GS.UIComponents.MenuNumberPicker.prototype = {
    ...
    init: function() {
        ...
        labelOffset.x += .5 * this.size.x,
        labelOffset.y += .5 * this.size.y,
...
GS.UIComponents.MenuPanel.prototype = {
    ...
    addButton: function(text, onClick) {
        ...
        return button.fontSize = this.fontSize,
    ...
    addToggleButton: function(text, states, buttonSize, onClick) {
        ...
        labelOffset.x += .5 * this.size.x - 10,
        labelOffset.y += .5 * this.rowHeight;
        ...
        buttonOffset.x += .5 * this.size.x + 10,
    ...
    addDoubleLabel: function(text1, text2) {
        ...
        label1Offset.x += .5 * this.size.x - 30,
        label1Offset.y += .5 * this.rowHeight;
        ...
        label2Offset.x += .5 * this.size.x + 30,
        label2Offset.y += .5 * this.rowHeight;
        var label2 = new GS.UIComponents.MenuLabel(this.cvs,text2,label2Offset,this.pos);
        return label2.fontSize = this.fontSize,
    ...
    addNumberPicker: function(text, value, min, max, step, onChange) {
        ...
        return this.children.push(numberPicker),
        this.rowOffset += this.rowHeight,
        {
            label: label,
    ...
    addImageButton: function(text, image, onClick) {
        ...
        return this.children.push(imageButton),
        this.rowOffset += this.rowHeight,
        imageButton
    },
    ...
    getRowOffset: function() {
        ...
        return offset.y += this.rowOffset,
        Math.abs(offset.y - this.offset.y) > this.size.y ? void GAME.handleFatalError("menu panel exceeds height") : offset
    },
    update: function() {
        for (var i = 0; i < this.children.length; i++)
            this.children[i].update()
...
GS.UIComponents.Notifications = function(vectorCanvas, assets, player) {
    ...
    this.calculateSizes(),
    this.visible = !0,
    this.oldShowUsePopup = !1,
    this.showUsePopup = !1,
    this.oldShowPointerLockPopup = !1,
    this.showPointerLockPopup = !1,
    this.oldShowRestartPopup = !1,
    this.showRestartPopup = !1
}
,
GS.UIComponents.Notifications.prototype = {
    ...
    update: function() {
        ...
        this.showUsePopup != this.oldShowUsePopup && (this.needsRedraw = !0,
        this.oldShowUsePopup = this.showUsePopup),
        this.showPointerLockPopup = !this.player.controls.pointerLockEnabled,
        this.showPointerLockPopup != this.oldShowPointerLockPopup && (this.needsRedraw = !0,
        this.oldShowPointerLockPopup = this.showPointerLockPopup),
        this.showRestartPopup = this.player.dead,
        this.showRestartPopup != this.oldShowRestartPopup && (this.needsRedraw = !0,
    ...
    drawPopup: function(text, popup) {
        this.cvs.roundedBoxFill(popup.offset, popup.pos, popup.size, !0, this.boxCornerRadius, GS.UIColors.background),
        this.cvs.text(popup.textOffset, popup.pos, text, GS.UIColors.foreground, this.fontSize, "middle", "center", GS.UIFont)
    },
    ...
    calculatePopupSize: function(text, popup, textWidth) {
        ...
        popup.textOffset.copy(popup.offset).add(popup.size.clone().multiplyScalar(.5))
    }
},
GS.UIComponents.PowerBars = function(vectorCanvas, assets, player) {
    ...
    this.healthBar.textOffset.add(this.healthBar.offset).add(this.healthBar.size.clone().multiplyScalar(.5)),
    this.ammoBar = {
        offset: new THREE.Vector2((-20),(-20)),
    ...
    this.ammoBar.textOffset.add(this.ammoBar.offset).add(this.ammoBar.size.clone().multiplyScalar(.5)),
...
GS.UIComponents.PowerBars.prototype = {
    ...
    update: function() {
        ...
        void 0 === this.player.weapon || this.player.weapon.infiniteAmmo ? this.ammo = void 0 : this.ammo = this.player.weapon.ammo,
        this.health == this.oldHealth && this.ammo == this.oldAmmo || (this.needsRedraw = !0,
        this.oldHealth = this.health,
        this.oldAmmo = this.ammo)
    },
    draw: function() {
        this.cvs.roundedBoxFill(this.healthBar.offset, this.healthBar.pos, this.healthBar.size, !0, this.boxCornerRadius, GS.UIColors.background),
        this.cvs.text(this.healthBar.textOffset, this.healthBar.pos, this.health, GS.UIColors.foreground, this.fontSize, "middle", "right", GS.UIFont),
        ...
        void 0 !== this.ammo && (this.cvs.roundedBoxFill(this.ammoBar.offset, this.ammoBar.pos, this.ammoBar.size, !0, this.boxCornerRadius, GS.UIColors.background),
        this.cvs.text(this.ammoBar.textOffset, this.ammoBar.pos, this.ammo, GS.UIColors.foreground, this.fontSize, "middle", "right", GS.UIFont),
        this.cvs.drawImageFromAtlas(this.ammoBar.imageOffset, this.ammoBar.pos, this.tex, this.ammoBar.imageAtlasOffset, this.ammoBar.imageAtlasSize))
    }
},
...
GS.UIComponents.WinScreen.prototype = {
    ...
    init: function() {},
    update: function() {
        !this.visible && void 0 !== this.aiManager.script && this.aiManager.script.mapWon && (this.collectData(),
        this.visible = !0)
    },
    collectData: function() {
        var script = this.aiManager.script;
        this.mapName = script.mapName,
        this.hasNextMap = void 0 !== script.nextMap,
    ...
    draw: function() {
        ...
        this.hasNextMap ? this.drawMessage("[ENTER] to continue to the next level", 11) : (this.drawMessage("congrats!", 10),
    ...
    drawField: function() {
        ...
        return function(name, value, row) {
            ...
            this.cvs.text(left, this.popup.pos, name, GS.UIColors.foreground, this.fontSize, "middle", "right", GS.UIFont),
            this.cvs.text(right, this.popup.pos, value, GS.UIColors.foreground, this.fontSize, "middle", "left", GS.UIFont)
        }
    }(),
    drawMessage: function() {
        ...
        return function(message, row) {
            ...
            this.cvs.text(center, this.popup.pos, message, GS.UIColors.foreground, this.fontSize, "middle", "center", GS.UIFont)
        }
    }()
},
GS.CollisionHelper = {
    ...
    collisionSlide: function(p, triangleIterator) {
        ...
        0 === p.gravityFactor || climbing || p.horizontalOnly || (p.lspVelocity.copy(this.gravity),
        ...
        p.horizontalOnly && (finalPosition.y = p.originalHeight);
        var result = {
            pos: finalPosition,
            climbing: climbing,
    ...
    collideWithWorld: function() {
        ...
        return function(p, triangleIterator) {
            ...
            if (p.lspNormalizedVelocity.copy(p.lspVelocity).normalize(),
            p.foundCollision = !1,
            p.nearestDistance = 0,
            ...
            !p.foundCollision)
                return result.copy(p.lspPosition).add(p.lspVelocity),
                result;
            if (destinationPoint.copy(p.lspPosition).add(p.lspVelocity),
            newPosition.copy(p.lspPosition),
            p.nearestDistance >= veryCloseDistance && (aux.copy(p.lspVelocity).normalize(),
            ...
            p.stopOnFirstHit)
                return p.lspPosition.copy(newPosition),
                p.lspPosition;
            ...
            return aux.copy(slidePlaneNormal).multiplyScalar(signedDistFromDestPointToSlidingPlane),
            newDestinationPoint.copy(destinationPoint).sub(aux),
            newVelocityVector.copy(newDestinationPoint).sub(p.intersectionPoint),
            newVelocityVector.length() < veryCloseDistance ? newPosition : (p.collisionRecursionDepth++,
    ...
    sphereCollidingWithTriangle: function() {
        ...
        return function(gridObject, p, v0, v1, v2, normal) {
            ...
            if (facing <= 0) {
                ...
                var t0, t1, sphereInPlane = !1, A = normal.x, B = normal.y, C = normal.z, D = -(A * v0.x + B * v0.y + C * v0.z), planeConstant = D, signedDistFromPositionToTriPlane = position.dot(normal) + planeConstant, planeNormalDotVelocity = normal.dot(velocity);
                if (0 === planeNormalDotVelocity) {
                    if (Math.abs(signedDistFromPositionToTriPlane) >= 1)
                        return !1;
                    sphereInPlane = !0
                } else {
                    if (t0 = (1 - signedDistFromPositionToTriPlane) / planeNormalDotVelocity,
                ...
                var collidingWithTri = !1
                  , t = 1;
                if (sphereInPlane || (aux.copy(velocity).multiplyScalar(t0),
                planeIntersectionPoint.copy(position).add(aux).sub(normal),
                this.checkPointInTriangle(planeIntersectionPoint, v0, v1, v2) && (collidingWithTri = !0,
                t = t0,
                ...
                !collidingWithTri) {
                    ...
                    c = aux.length(),
                    c = c * c - 1,
                    this.getLowestRoot(a, b, c, t, result) && (t = result.root,
                    collidingWithTri = !0,
                    ...
                    if (a = edgeLengthSquared * -velocityLengthSquared + edgeDotVelocity * edgeDotVelocity,
                    b = 2 * edgeLengthSquared * velocity.dot(spherePositionToVertex) - 2 * edgeDotVelocity * edgeDotSpherePositionToVertex,
                    c = edgeLengthSquared * (1 - spherePositionToVertexLengthSquared) + edgeDotSpherePositionToVertex * edgeDotSpherePositionToVertex,
                    this.getLowestRoot(a, b, c, t, result)) {
                        var f = (edgeDotVelocity * result.root - edgeDotSpherePositionToVertex) / edgeLengthSquared;
                        f >= 0 && f <= 1 && (t = result.root,
                        collidingWithTri = !0,
                        edge.multiplyScalar(f),
                        collisionPoint.copy(v0).add(edge))
                    }
                    if (edge.copy(v2).sub(v1),
                ...
                if (collidingWithTri) {
                    ...
                    if (!p.foundCollision || distToCollision < p.nearestDistance)
                        return p.nearestDistance = distToCollision,
    ...
    getLowestRoot: function(a, b, c, maxR, result) {
        ...
        return r1 > 0 && r1 < maxR ? (result.root = r1,
        !0) : r2 > 0 && r2 < maxR && (result.root = r2,
        !0)
    }
},
GS.HitscanHelper = {
        ...
        return function(pos, dir, grid, typesEnvironment, typesEntity, steps, except) {
            ...
            for (var i = 0; i < steps; i++) {
                ...
                if (void 0 === gridLocation)
                    break;
                if (this.checkEnvironmentIntersection(ray, grid, gridLocation, typesEnvironment, result, except),
                this.checkEntityIntersection(ray, grid, gridLocation, typesEntity, result, except),
                result.type !== GS.CollisionTypes.None)
                    break;
                newPos.add(step)
            }
            return result
        ...
        return function(ray, grid, gridLocation, types, result, except) {
            ...
            triangleIterator(function(gridObject, v0, v1, v2) {
                if (gridObject !== except && null !== ray.intersectTriangle(v0, v1, v2, !0, point)) {
                    ...
                    dist < result.distance && (result.type = GS.CollisionTypes.Environment,
                    aux.subVectors(v2, v0),
        ...
        return function(ray, grid, gridLocation, types, result, except) {
            ...
            grid.forEachUniqueGridObjectInCells(cells, types, function(gridObject) {
                if (!gridObject.dead && gridObject !== except && null !== ray.intersectBox(gridObject.view.collisionData.boundingBox, point)) {
                    var dist = point.distanceToSquared(ray.origin);
                    dist < result.distance && (result.type = GS.CollisionTypes.Entity,
...
GS.RegionHelper.prototype = {
    ...
    getRegions: function(sectors, sectorLinks) {
        ...
        return result.regions = this.constructRegions(result.sectorDict, result.sectorGraph),
        result
    },
        ...
        if (doorCount > 0) {
            for (var visited = {}; toVisit.length > 0; ) {
                var current = toVisit.pop()
                  , id = current.id
                  , seedId = current.seed;
                if (sectorDict[id].sector.door && (seeds[seedId].doorIds[id] = !0),
                !visited[id] && (visited[id] = !0,
                seeds[seedId].sectorIds[id] = !0,
                !sectorDict[id].sector.door))
    ...
    getNewRegion: function(doorIds, sectorIds) {
        ...
        return mesh.matrixAutoUpdate = !1,
        region.mesh = mesh,
        this.regionIdCount++,
        region
    },
...
GS.MeshPhongGlowMaterial = function(map, glow, normal, emissive, glowIntensity) {
    ...
    this.uniforms.glowIntensity.value = void 0 !== glowIntensity ? glowIntensity : 1,
    ...
    this.map = !0,
    this.normalMap = void 0 !== normal && null !== normal
}
,
...
GS.GridObject = function(grid, layer, sourceObj) {
    ...
    this.removed = !1,
    this.usable = !1,
    this.lightLevelFactor = .1,
    this.minLightLevel = .1,
    this.maxLightLevel = .75,
...
GS.GridObject.prototype = {
    ...
    updateBoundingBox: function() {
        ...
        return function() {
            ...
            void 0 !== debugMesh && (debugMesh.userData.isTriangleMesh !== !0 ? (debugMesh.scale.subVectors(max, min),
            debugMesh.position.copy(this.position)) : (debugMesh.geometry.vertices.length = 0,
            GS.pushArray(debugMesh.geometry.vertices, data.triangles),
            debugMesh.geometry.verticesNeedUpdate = !0)),
            void 0 !== data.boundingSquare && (this.size.toVector2(aux),
    ...
    updateTriangles: function(velocity) {
        for (var triangles = this.view.collisionData.triangles, i = 0; i < triangles.length; i++)
            triangles[i].add(velocity)
    },
    updateSegments: function(yVelocity) {
        for (var segs = this.view.collisionData.segments, i = 0; i < segs.length; i++)
            segs[i].bottomY += yVelocity,
            segs[i].topY += yVelocity
    },
    assignToCells: function(gridLocation) {
        void 0 !== gridLocation ? void 0 !== this.gridLocation ? gridLocation.gridMin.equals(this.gridLocation.gridMin) && gridLocation.gridMax.equals(this.gridLocation.gridMax) || (this.unlinkFromCells(),
        this.linkToCells(gridLocation)) : this.linkToCells(gridLocation) : this.unlinkFromCells()
    },
    ...
    getLightColorFromSector: function(optionalTarget, optionalSector) {
        ...
        if (void 0 !== sector) {
            ...
            color.setHex(16777215).multiplyScalar(x)
        }
    },
    ...
    getSector: function() {
        ...
        return function() {
            ...
            return this.position.toVector2(position2d),
...
GS.Concrete = function(grid, layer, sourceObj) {
    ...
    layer === GS.MapLayers.Sector && (this.sector = this.sourceObj),
    this.view = {
        collisionData: {
            triangles: null
        }
...
GS.Door = function(grid, sector) {
    ...
    this.maxCooldown = GS.msToFrames(5e3),
...
GS.Door.prototype = GS.inherit(GS.GridObject, {
    ...
    update: function() {
        ...
        case GS.DoorStates.Open:
            this.automatic ? (this.cooldown--,
            this.cooldown <= 0 && (this.cooldown = this.maxCooldown,
            this.state = GS.DoorStates.Closing,
            this.grid.soundManager.playSound("door_close"))) : this.cooldown = this.maxCooldown;
            break;
        ...
        case GS.DoorStates.Opening:
            ...
            this.position.y >= this.openY && (this.position.y = this.openY,
            this.state = GS.DoorStates.Open);
            break;
        case GS.DoorStates.Closing:
            this.grid.collisionManager.isEntityNearDoor(this) ? (this.sector.doorOpenedEver = !0,
            this.state = GS.DoorStates.Opening) : (this.position.y -= this.speed,
            velocity.y = -this.speed,
            this.position.y <= this.closedY && (this.position.y = this.closedY,
            this.state = GS.DoorStates.Closed))
        }
        if (this.usable = this.state === GS.DoorStates.Closed && this._automatic === !0,
        0 !== velocity.y && (this.updateBoundingBox(),
        this.updateTriangles(velocity),
        this.updateSegments(velocity.y),
        this.updateMesh()),
        !this.sector.useVertexColors) {
    ...
    close: function() {
        return this.state = GS.DoorStates.Closing,
        this.grid.soundManager.playSound("door_close"),
        this
    },
    set automatic(value) {
        this._automatic = value,
        this.usable = this._automatic === !0
    },
    get automatic() {
...
GS.Elevator.prototype = GS.inherit(GS.GridObject, {
    ...
    update: function() {
        ...
        case GS.ElevatorStates.GoingDown:
            ...
            this.position.y -= this.speed,
            this.position.y <= this.downY ? (this.position.y = this.downY,
            this.velocity.y = this.downY - oldY,
            this.state = GS.ElevatorStates.Down,
            this.grid.soundManager.playSound("elevator_stop")) : this.velocity.y = -this.speed;
            break;
        case GS.ElevatorStates.GoingUp:
            oldY = this.position.y,
            this.position.y += this.speed,
            this.position.y >= this.upY ? (this.position.y = this.upY,
    ...
    updateChange: function() {
        if (0 !== this.velocity.y && (this.updateBoundingBox(),
    ...
    setPositionUp: function() {
        return this.velocity.y = this.upY - this.position.y,
    ...
    setPositionDown: function() {
        return this.velocity.y = this.downY - this.position.y,
    ...
    goDown: function() {
        return this.state = GS.ElevatorStates.GoingDown,
        this.grid.soundManager.playSound("elevator_move"),
        this
    },
    goUp: function() {
        return this.state = GS.ElevatorStates.GoingUp,
        this.grid.soundManager.playSound("elevator_move"),
        this
    },
    updateMesh: function() {
...
GS.Item = function(grid, layer, sourceObj) {
    ...
    this.scale = new THREE.Vector3(.4,.4,.4),
...
GS.Monster = function(grid, layer, sourceObj) {
    ...
    $.extend(!0, this, GS.Monsters[this.monsterType]),
...
GS.Monster.prototype = GS.inherit(GS.GridObject, {
    ...
    update: function() {
        ...
        this.dead || (this.state === GS.MonsterStates.Awake ? this.updateScan() : this.state === GS.MonsterStates.Active && (this.updateMove(),
        this.attackType === GS.MonsterAttackTypes.Melee ? this.updateAttackMelee() : this.attackType === GS.MonsterAttackTypes.Ranged && this.updateAttackRanged())),
        this.updateLightLevel()
    },
    updateLightLevel: function() {
    ...
    updateMove: function() {
        var target = this.grid.player;
        this.chargingUpRangedAttack ? (this.calculateDirection(target.position),
        this.calculateRotation(),
        this.updateMesh()) : (this.scatterCooldown > 0 ? this.scatterCooldown-- : target.dead ? this.scatter() : this.attackType === GS.MonsterAttackTypes.Melee ? this.updateMoveMelee() : this.attackType === GS.MonsterAttackTypes.Ranged && this.updateMoveRanged(),
        this.inPain ? this.updateMesh() : this.move())
    },
    updateMoveMelee: function() {
        ...
        return function() {
            var target = this.grid.player;
            if (this.changeTargetCooldown > 0 && (this.changeTargetCooldown--,
            0 === this.changeTargetCooldown)) {
                ...
                distanceToTarget > 20 && (aux.set(Math.random() - .5, 0, Math.random() - .5).normalize().multiplyScalar(20),
                targetPos.add(aux)),
    ...
    updateMoveRanged: function() {
        ...
        return function() {
            ...
            0 === this.changeTargetCooldown)) {
                ...
                targetPos.copy(target.position);
                var distanceToTarget = this.position.distanceTo(targetPos);
                distanceToTarget > 30 && distanceToTarget < this.preferredMaxDistance && (targetPos.copy(this.position),
                aux.set(Math.random() - .5, 0, Math.random() - .5).normalize().multiplyScalar(20),
                targetPos.add(aux)),
                this.calculateDirection(targetPos),
                this.calculateRotation()
            }
    ...
    updateAttackRanged: function() {
        var target = this.grid.player;
        this.rangedAttackCooldown > 0 ? this.rangedAttackCooldown-- : target.dead || this.inPain || (this.rangedAttackChargeCooldown > 0 ? (this.rangedAttackChargeCooldown--,
        0 === this.rangedAttackChargeCooldown && this.rangedAttack()) : this.isFacing(target.position) && this.grid.collisionManager.checkMonsterLineOfSight(this, target, this.rangedAttackRange) && this.chargeUpRangedAttack())
    },
    chargeUpRangedAttack: function() {
        this.grid.soundManager.playSound(this.rangedAttackChargeUpSound),
        this.moving = !1,
        this.chargingUpRangedAttack = !0,
    ...
    rangedAttack: function() {
        ...
        return function() {
            ...
            this.chargingUpRangedAttack = !1,
            this.animationView.setLoop("walk"),
            this.rangedAttackCooldown = this.rangedAttackMaxCooldown + Math.floor(Math.random() * this.rangedAttackCooldownRandomModifier),
    ...
    isFacing: function() {
        ...
        return function(pos) {
            ...
            return a.set(Math.sin(x - Math.PI / 2), Math.cos(x - Math.PI / 2)).add(p),
            b.set(Math.sin(x + Math.PI / 2), Math.cos(x + Math.PI / 2)).add(p),
            GS.MathHelper.vec2PointSide(a, b, t)
        }
    }(),
    scatter: function() {
        var target = this.grid.player;
        if (!(this.scatterCooldown > 20)) {
            ...
            (target.dead || distance > 10 * (1 + this.scale.x)) && (this.direction = new THREE.Vector3(2 * Math.random() - 1,0,2 * Math.random() - 1),
    ...
    updateCollisionData: function() {
        ...
        return function(newPos) {
            ...
            this.updateBoundingBox(),
            this.updateMesh(),
            currentSpeed / this.speed < .1 && this.scatter(),
            this.sector = this.getSector()
        }
    }(),
    wakeUp: function(script) {
        this.state !== GS.MonsterStates.Awake && (script !== !0 && this.state === GS.MonsterStates.Scripted || (this.state = GS.MonsterStates.Awake,
        this.animationView.setLoop("inactive")))
    },
    activate: function(script) {
        this.state !== GS.MonsterStates.Active && (script !== !0 && this.state === GS.MonsterStates.Scripted || (this.state = GS.MonsterStates.Active,
        this.animationView.setLoop("walk"),
        this.grid.soundManager.playSound(this.roarSound)))
    },
    onHit: function(damage) {
        this.state !== GS.MonsterStates.Active && this.activate(!0),
        Math.random() <= this.painChance && (this.inPain = !0,
        this.moving = !1,
        this.attackType === GS.MonsterAttackTypes.Ranged && this.cancelRangedAttack(),
...
GS.Player.prototype = GS.inherit(GS.GridObject, {
    ...
    init: function() {
        var that = this;
        this.availableWeapons = $.extend(!0, {}, GS.Weapons),
        Object.keys(this.availableWeapons).forEach(function(key) {
            that.availableWeapons[key].pickedUp = !1,
        ...
        ,
        that.availableWeapons.pistol.pickedUp = !0,
    ...
    giveAll: function() {
        ...
        Object.keys(this.availableWeapons).forEach(function(key) {
            ...
            weapon.infiniteAmmo || (weapon.ammo = weapon.ammoMax)
        })
    },
    fly: function(value) {
        void 0 === value ? this.flyEnabled = !this.flyEnabled : this.flyEnabled = value,
        this.controls.fly = this.flyEnabled,
        this.flyEnabled ? (this.playerView.viewBob.enabled = !1,
        this.playerView.weaponBob.enabled = !1) : (this.playerView.viewBob.enabled = !0,
        this.playerView.weaponBob.enabled = !0)
    },
    ...
    initTriangles: function() {
        for (var triangles = GS.Cube.getVertices(), i = 0; i < triangles.length; i++)
    ...
    onHit: function(damage) {
        this.godEnabled || (this.health -= damage,
        this.health <= 0 && (this.health = 0,
        this.onDeath()),
    ...
    onDeath: function() {
        ...
        void 0 !== this.weapon && this.playerView.onDeath()
    },
    onItemCollision: function(item) {
        var name = GS.MapEntities[item.sourceObj.type].name;
        (this.pickupWeapon(name) || this.pickupAmmo(name) || this.pickupMedkit(name)) && (this.grid.aiManager.onPlayerItemPickup(this, item),
        item.remove(),
    ...
    pickupWeapon: function(name) {
        if (void 0 !== this.availableWeapons[name]) {
            ...
            return weapon.pickedUp || (weapon.pickedUp = !0,
            GS.DebugUI.addTempLine("picked up " + weapon.name),
            (void 0 === this.weapon || this.swapWeaponsOnPickup && weapon.powerLevel > this.weapon.powerLevel) && (this.weapon = weapon,
            this.weaponName = name,
            this.playerView.changeToWeapon(name))),
            weapon.ammo += weapon.ammoClip,
            this.grid.soundManager.playSound("pickup_weapon"),
            weapon.ammo > weapon.ammoMax && (weapon.ammo = weapon.ammoMax),
    ...
    pickupAmmo: function(name) {
        ...
        return !("ammo" != name || (Object.keys(this.availableWeapons).forEach(function(key) {
            var weapon = that.availableWeapons[key];
            !weapon.infiniteAmmo && weapon.ammo < weapon.ammoMax && (weapon.ammo += weapon.ammoClip,
            weapon.ammo > weapon.ammoMax && (weapon.ammo = weapon.ammoMax),
            GS.DebugUI.addTempLine("picked up " + weapon.ammoClip + " " + weapon.name + " ammo"),
            once = !0)
        }),
        !once)) && (this.grid.soundManager.playSound("pickup_ammo"),
        !0)
    },
    pickupMedkit: function(name) {
        return "medkit" == name && this.health < 100 && (GS.DebugUI.addTempLine("picked up medkit"),
        this.health += 25,
    ...
    updateShoot: function() {
        if (void 0 !== this.weapon) {
            if (this.weapon.cooldownRemaining > 0)
                return void (this.weapon.cooldownRemaining -= 1);
            ...
            if (GS.Keybinds.shoot.inUse && this.playerView.weaponReady) {
                if (this.weapon.infiniteAmmo || this.weapon.ammo > 0) {
                    this.shooting = !0,
                    this.shooting && !oldShooting && this.shootStart(),
                    this.weapon.infiniteAmmo || (this.weapon.ammo -= this.weapon.bulletsPerShot),
                ...
                this.shooting = !1,
                !this.shooting && oldShooting && this.shootEnd()
        }
    },
    playShootSound: function() {
        var name;
        "double_shotgun" == this.weaponName ? name = "shotgun_fire" : "hyper_blaster" == this.weaponName ? name = "hyper_blaster_fire" : "pistol" == this.weaponName && (name = "hyper_blaster_fire"),
    ...
    getProjectileStart: function() {
        ...
        return projectileStart.y += this.controls.eyeOffsetY,
        projectileStart
    },
    ...
    updateControls: function(e) {
        this.ySmoothingCooldown > 0 ? (this.ySmoothingCooldown--,
        this.controls.ySmoothing = !0) : this.controls.ySmoothing = !1,
    ...
    updateCollisionData: function(newPos) {
        ...
        this.moving && !oldMoving ? this.moveStart() : !this.moving && oldMoving && this.moveEnd(),
        this.updateBoundingBox()
    }
}),
GS.Projectile = function(grid, sourceGridObject, type, position, direction) {
    GS.GridObject.call(this, grid),
    $.extend(!0, this, GS.Projectiles[type]),
...
GS.Projectile.prototype = GS.inherit(GS.GridObject, {
    ...
    update: function() {
        ...
        this.view.mesh.position.copy(this.position),
        this.rotating && (this.view.mesh.rotation.x += .1,
        this.view.mesh.rotation.y += .1),
        (this.steps > this.maxSteps || 0 === this.linkedGridCells.length) && this.remove()
    },
    updateCollisionData: function(newPos) {
        this.distanceTravelled += newPos.distanceTo(this.position),
        this.steps > 1 && (this.view.mesh.visible = !0),
        this.position.copy(newPos),
        this.updateBoundingBox()
    },
    onHit: function() {
...
GS.FontLoader.prototype = {
    ...
    load: function(name, filename, callback) {
        ...
        var intervalId = setInterval(function() {
            node.offsetWidth != width && (node.parentNode.removeChild(node),
...
GS.MapLoader.prototype = {
    ...
    loadTestMap: function() {
        ...
        return testMap || alert("testMap not found in localStorage"),
        testMap
    },
    load: function(name, filename, callback) {
    ...
    parse: function(text) {
        ...
        try {
            map = JSON.parse(text, function(k, v) {
                if (v instanceof Object) {
                    if (void 0 !== v.x && void 0 !== v.y && void 0 !== v.z)
                        return new THREE.Vector3(v.x,v.y,v.z);
                    if (void 0 !== v.x && void 0 !== v.y)
        ...
        return map.bounds = this.getMapBounds(map),
        map
    },
    getMapBounds: function(map) {
        ...
        return bounds.setFromPoints(points),
        bounds
    }
},
...
GS.GridFactory.prototype = {
    ...
    getRegions: function(map) {
        ...
        if (void 0 === sectorLinks)
            return void GAME.handleFatalError("invalid map - sector links not found");
    ...
    addPlayerToGrid: function(grid) {
        ...
        position.y = 0,
        position.z = grid.map.playerStartPosition.y - 1e-4,
    ...
    assignMapEntitiesToGrid: function(grid) {
        for (var sectorDict = {}, sectors = grid.map.layerObjects[GS.MapLayers.Sector], i = 0; i < sectors.length; i++) {
            ...
            if (hasFloor || hasCeiling) {
                if (hasFloor) {
                    ...
                    this.viewFactory.applySectorView(floor, !1),
                ...
                if (hasCeiling) {
                    ...
                    this.viewFactory.applySectorView(ceiling, !0),
        ...
        for (var segs = grid.map.layerObjects[GS.MapLayers.Segment], i = 0; i < segs.length; i++) {
            ...
            if (seg.type !== GS.SegmentTypes.TVScreen && seg.type !== GS.SegmentTypes.Switch) {
                ...
                seg.type === GS.SegmentTypes.TVScreen ? this.addTVScreen(grid, gridLocation, seg) : seg.type === GS.SegmentTypes.Switch && this.addSwitch(grid, gridLocation, seg)
        }
...
GS.ViewFactory = function(renderer, map, assets) {
    ...
    this.debugBoundingBoxMaterial = new THREE.MeshBasicMaterial({
        color: 16777215,
        wireframe: !0
    }),
...
GS.ViewFactory.prototype = {
    ...
    init: function() {
        ...
        for (var i in textureAssets) {
            ...
            tex.type === GS.TextureTypes.Map ? (this.wrap(this.textures[i]),
        ...
        for (var entities = this.map.layerObjects[GS.MapLayers.Entity], entityMaterials = {}, i = 0; i < entities.length; i++) {
            var ntt = entities[i]
              ...
              , name = desc.name;
            void 0 === entityMaterials[name] && (entityMaterials[name] = !0,
    ...
    getEmissiveColor: function(lightLevel) {
        ...
        new THREE.Color(16777215).multiplyScalar(x)
    },
    applySectorView: function(gridObject, ceiling) {
        var geometry, sector = gridObject.sourceObj, triangles = [];
        if (ceiling) {
            var hasCeiling = void 0 === sector.ceiling || sector.ceiling;
            hasCeiling && (geometry = this.getSectorGeometry(sector, triangles, !0),
            gridObject.region.mesh.geometry.merge(geometry))
        } else
            sector.elevator !== !0 && (geometry = this.getSectorGeometry(sector, triangles, !1),
            gridObject.region.mesh.geometry.merge(geometry));
    ...
    applyConcreteBoundingBox: function(gridObject, triangles) {
        ...
        debugMesh.scale.copy(boundingBox.max).sub(boundingBox.min).addScalar(.01),
    ...
    getSectorGeometry: function(sector, sectorTriangles, ceiling) {
        ...
        GS.pushArray(geometry.vertices, triangles),
        GS.pushArray(sectorTriangles, this.getSectorTriangles(sector, ceiling, !0));
    ...
    getSegmentDefinedTriangles: function(seg) {
        for (var triangles = [], bottomRightIndex = 0, i = 0; i < seg.indices.length; i += 3)
            ...
            seg.indices[i + 2] == seg.bottomRightIndex && (bottomRightIndex = i + 3);
        return seg.bottomRightIndex = bottomRightIndex,
        triangles
    },
    getSegmentTriangles: function(seg) {
        var triangles = [];
        return triangles.push(new THREE.Vector3(seg.start.x,seg.topY,seg.start.y)),
    ...
    getSectorTriangles: function(sector, ceiling, getCollisionTriangles) {
        var triangles = []
          , v = sector.vertices
          , idx = sector.indices;
        if (getCollisionTriangles && (v = sector.collisionVertices,
    ...
    applyEntityView: function(gridObject) {
        ...
        return function(gridObject) {
            var mesh, that = this, ntt = gridObject.sourceObj, desc = GS.MapEntities[ntt.type], name = GS.MapEntities[ntt.type].name, offset = gridObject.offset, scale = gridObject.scale, size = gridObject.size, material = this.materials[name].clone();
            if (void 0 === desc.animations)
            ...
            gridObject instanceof GS.Monster) {
    ...
    getSegmentsForMovingSector: function(gridObject, elevator) {
        for (var sector = gridObject.sector, segs = [], i = 0; i < sector.collisionVertices.length - 1; i++)
            segs.push({
    ...
    applyDoorView: function(door) {
        ...
        door.view.collisionData.segments = this.getSegmentsForMovingSector(door, !1);
        var centerMaterial = this.vertexColorMaterial
          , sideMaterial = this.vertexColorMaterial;
        door.sector.useVertexColors || (centerMaterial = this.materials[door.sector.ceilTexId].clone(),
    ...
    applyElevatorView: function(elevator) {
        var triangles = [];
        elevator.position = this.getMovingSectorPosition(elevator, !0),
        elevator.view.collisionData.segments = this.getSegmentsForMovingSector(elevator, !0);
        var centerMaterial = this.vertexColorMaterial
          , sideMaterial = this.vertexColorMaterial;
        elevator.sector.useVertexColors || (centerMaterial = this.materials[elevator.sector.floorTexId].clone(),
        sideMaterial = this.materials[elevator.sector.sideTexId].clone());
        var mesh = new THREE.Object3D;
        mesh.children.push(this.getMovingSectorCenterMesh(elevator, triangles, centerMaterial, !0)),
        mesh.children.push(this.getMovingSectorSideMesh(elevator, triangles, sideMaterial, !0)),
    ...
    getMovingSectorPosition: function(movingSector, elevator) {
        ...
        return elevator !== !0 ? pos.y = (movingSector.sector.floorTopY + movingSector.sector.ceilBottomY) / 2 : pos.y = movingSector.sector.floorTopY - movingSector.sector.elevatorMaxHeight / 2,
        pos
    },
    getMovingSectorSideMesh: function(movingSector, movingSectorTriangles, material, elevator) {
        var bottomY, topY;
        elevator !== !0 ? (bottomY = movingSector.sector.floorTopY,
        topY = movingSector.sector.ceilBottomY) : (bottomY = movingSector.sector.floorTopY - movingSector.sector.elevatorMaxHeight,
        ...
        for (var geometry = new THREE.Geometry, color = new THREE.Color(movingSector.sector.lightColor), emissive = this.getEmissiveColor(movingSector.sector.lightLevel), triangles = [], k = 0, i = 0; i < movingSector.sector.vertices.length; i++) {
            var length, next = i < movingSector.sector.vertices.length - 1 ? i + 1 : 0, start = movingSector.sector.vertices[i], end = movingSector.sector.vertices[next], distance = start.distanceTo(end);
            length = distance > this.texScale ? Math.round(distance / (this.texScale * this.texScale) * this.texScale) : distance / this.texScale;
            for (var vertices = [new THREE.Vector3(start.x,topY,start.y), new THREE.Vector3(end.x,topY,end.y), new THREE.Vector3(start.x,bottomY,start.y), new THREE.Vector3(end.x,topY,end.y), new THREE.Vector3(end.x,bottomY,end.y), new THREE.Vector3(start.x,bottomY,start.y)], j = 0; j < vertices.length; j++)
                triangles.push(vertices[j].clone());
            for (var j = 0; j < vertices.length; j++)
                vertices[j].sub(movingSector.position);
            if (GS.pushArray(geometry.vertices, vertices),
            movingSector.sector.useVertexColors) {
                ...
                geometry.faceVertexUvs[0].push([new THREE.Vector2(0,topY / this.texScale), new THREE.Vector2(length,topY / this.texScale), new THREE.Vector2(0,bottomY / this.texScale)]),
                geometry.faceVertexUvs[0].push([new THREE.Vector2(length,topY / this.texScale), new THREE.Vector2(length,bottomY / this.texScale), new THREE.Vector2(0,bottomY / this.texScale)]);
            k += 6
        }
        if (GS.pushArray(movingSectorTriangles, triangles),
        geometry.computeFaceNormals(),
        geometry.computeVertexNormals(),
        movingSector.sector.useVertexColors) {
    ...
    getMovingSectorCenterMesh: function(movingSector, movingSectorTriangles, material, elevator) {
        for (var v0, v1, v2, minHeight = movingSector.sector.floorTopY, geometry = new THREE.Geometry, color = new THREE.Color(movingSector.sector.lightColor), emissive = this.getEmissiveColor(movingSector.sector.lightLevel), v = movingSector.sector.vertices, idx = movingSector.sector.indices, i = 0; i < idx.length; i += 3) {
            if (elevator === !0 ? (v0 = new THREE.Vector3(v[idx[i + 2]].x,minHeight,v[idx[i + 2]].y),
            v1 = new THREE.Vector3(v[idx[i + 1]].x,minHeight,v[idx[i + 1]].y),
            v2 = new THREE.Vector3(v[idx[i]].x,minHeight,v[idx[i]].y)) : (v0 = new THREE.Vector3(v[idx[i]].x,minHeight,v[idx[i]].y),
            v1 = new THREE.Vector3(v[idx[i + 1]].x,minHeight,v[idx[i + 1]].y),
            ...
            movingSector.sector.useVertexColors) {
                ...
                geometry.faceVertexUvs[0].push([new THREE.Vector2(v0.x / this.texScale,v0.z / this.texScale), new THREE.Vector2(v1.x / this.texScale,v1.z / this.texScale), new THREE.Vector2(v2.x / this.texScale,v2.z / this.texScale)]);
        ...
        for (var i = 0; i < idx.length; i += 3)
            elevator === !0 ? (v0 = new THREE.Vector3(v[idx[i + 2]].x,minHeight,v[idx[i + 2]].y),
            v1 = new THREE.Vector3(v[idx[i + 1]].x,minHeight,v[idx[i + 1]].y),
            v2 = new THREE.Vector3(v[idx[i]].x,minHeight,v[idx[i]].y)) : (v0 = new THREE.Vector3(v[idx[i]].x,minHeight,v[idx[i]].y),
            v1 = new THREE.Vector3(v[idx[i + 1]].x,minHeight,v[idx[i + 1]].y),
            v2 = new THREE.Vector3(v[idx[i + 2]].x,minHeight,v[idx[i + 2]].y)),
    ...
    applySwitchView: function(switchObj) {
        ...
        switchObj.view.collisionData.boundingBox.setFromPoints([new THREE.Vector3(switchObj.segment.start.x,switchObj.segment.bottomY,switchObj.segment.start.y), new THREE.Vector3(switchObj.segment.end.x,switchObj.segment.topY,switchObj.segment.end.y)]),
    ...
    getDebugMesh: function(position) {
        ...
        return debugMesh.position.copy(position),
        debugMesh
    },
    getDebugTriangleMesh: function(triangles) {
        ...
        return debugMesh.userData.isTriangleMesh = !0,
        debugMesh.matrixAutoUpdate = !1,
        debugMesh
    }
},
...
GS.AIManager.prototype = {
    ...
    assignMonstersToRegions: function() {
        ...
        this.grid.forEachUniqueGridObject([GS.Monster], function(monster) {
            if (void 0 === monster.startingSector)
                return void GAME.handleFatalError("monster " + monster.id + " has no starting sector");
    ...
    update: function() {
        void 0 !== this.script && (this.script.update(),
        this.script.mapWon && !this.mapWon && (this.mapWon = !0)),
        this.mapWon || this.updateTime()
    },
    updateTime: function() {
        this.grid.player.inMenu || (this.timeSpent += performance.now() - this.lastFrameTime,
        this.minutes = Math.floor(Math.floor(this.timeSpent) / 6e4),
        this.seconds = Math.floor(Math.floor(this.timeSpent) / 1e3 - 60 * this.minutes)),
        this.lastFrameTime = performance.now()
    },
    resume: function() {
        this.lastFrameTime = performance.now()
    ...
    onPlayerMove: function(player, oldPos, newPos) {
        void 0 !== this.script && this.checkZones(player, oldPos, newPos),
    ...
    onPlayerItemPickup: function(player, item) {
        this.itemsPickedUp++,
        void 0 !== this.script && this.script.onItemPickup(item)
    },
    ...
    applyRegionVisibility: function(player) {
        for (var i = 0; i < this.regions.length; i++)
            this.regions[i].reachedThisFrame = !1;
        var visibleRegions = 0;
        this.propagateRegions(player, function(region) {
            region.reachedThisFrame = !0,
            region.mesh.visible = !0,
            visibleRegions++
        });
        for (var i = 0; i < this.regions.length; i++) {
    ...
    propagateRegions: function(player, callback) {
        ...
        if (void 0 !== sector)
            for (var toVisit = [this.getRegionBySectorId(sector.id)], visited = {}; toVisit.length > 0; ) {
                ...
                if (!(region.id in visited) && (visited[region.id] = !0,
                this.regionsDiscovered[region.id] = region,
                callback(region),
                void 0 !== region.linkedRegions))
...
GS.ChromeAppFPSControls = function(camera) {
    ...
    this.moveSpeed = 1,
    this.lookSpeed = .066 * (GS.Settings.mouse / 5),
    ...
    this.viewOffsetY = 0,
    this.eyeOffsetY = 3.5,
    this.pointerLockEnabled = !1,
    this.onHandleCollisions = function(oldPos, newPos) {}
    ,
    this.enabled = !0,
    this.fly = !1,
    this.ySmoothing = !1,
    this.ySmoothingFactor = .1
}
,
GS.ChromeAppFPSControls.prototype = {
    ...
    init: function() {
        ...
        return pointerLock ? (this.canvas = $("#game-canvas")[0],
        this.debugCanvas = $("#debug-ui-canvas")[0],
        void this.setViewAngles(this.xAngle, this.yAngle)) : void GAME.handleFatalError("Pointer Lock API not supported")
    },
    enable: function() {
        var that = this;
        this.canvas.requestPointerLock(),
        $(document).off("mousemove.fpsControls"),
        $(document).on("mousemove.fpsControls", function(e) {
    ...
    update: function() {
        if (this.enabled) {
            var x = 0
              , z = 0;
            GS.Keybinds.moveForward.inUse && z++,
            GS.Keybinds.moveBackward.inUse && z--,
            GS.Keybinds.strafeLeft.inUse && x--,
            GS.Keybinds.strafeRight.inUse && x++,
            this.move(x, z),
            this.dispatchEvent({
                type: "update",
                pos: this.eye,
    ...
    move: function() {
        ...
        return function(x, z) {
            ...
            0 === x && 0 === z || (aux.copy(this.rightMoveVector).multiplyScalar(this.moveSpeed * x),
            newPos.add(aux),
    ...
    updateCamera: function() {
        ...
        this.ySmoothing ? this.followingEye.y != this.eye.y && (this.followingEye.y = this.followingEye.y + this.ySmoothingFactor * (this.eye.y - this.followingEye.y)) : this.followingEye.y = this.eye.y,
    ...
    setViewAngles: function() {
        ...
        return function(x, y) {
            ...
            this.fly ? this.forwardMoveVector.copy(forward).normalize() : this.forwardMoveVector.set(forward.x, 0, forward.z).normalize(),
    ...
    dispose: function(callback) {
        this.onHandleCollisions = void 0,
        this.disable()
    }
},
...
GS.CollisionManager.prototype = {
    ...
    collidePlayerEnvironment: function() {
        ...
        return function(player, oldPos, newPos) {
            ...
            if (points3d[0].copy(oldPos).sub(player.size),
            ...
            velocityBoundingBox.expandByScalar(2 * this.gravity),
            void 0 !== gridLocation) {
    ...
    slidingBoxLineCollision: function() {
        ...
        return function(oldPos, newPos, box, lineSegmentIterator) {
            ...
            newPos0.copy(newPos);
            for (var foundCollision, k = 0; k < 5 && (foundCollision = !1,
            ...
            if (void 0 !== gridLocation) {
                ...
                this.grid.forEachUniqueGridObjectInCells(cells, usableTypes, function(gridObject) {
                    if (null !== ray.intersectsOrStartsInBox(gridObject.view.collisionData.boundingBox, aux)) {
                        ...
                        dist <= player.useRange && dist < minDist && (minDist = dist,
                        minObject = gridObject)
                    }
                }),
                player.useTarget = minObject,
                player.canUse = null !== minObject && minObject.usable
            }
        }
    }(),
    ...
    collideMonsterPlayerEnvironment: function() {
        ...
        return function(monster, oldPos, newPos) {
            ...
            points[3].set(newPos.x - lsp.x, newPos.z - lsp.z);
            var gridLocation = this.grid.getGridLocationFromPoints(points);
            if (void 0 !== gridLocation && (this.handleConcreteCollisions(gridLocation, oldPos, newPos, monster.size),
            points[0].set(oldPos.x + lsp.x, oldPos.z + lsp.z),
            points[1].set(oldPos.x - lsp.x, oldPos.z - lsp.z),
            points[2].set(newPos.x + lsp.x, newPos.z + lsp.z),
            points[3].set(newPos.x - lsp.x, newPos.z - lsp.z),
            ...
            void 0 !== gridLocation)) {
                ...
                if (this.grid.forEachUniqueGridObjectInCells(cells, types, function(gridObject) {
                    foundCollision || gridObject === monster || gridObject instanceof GS.Monster && gridObject.dead || velocityBox.isIntersectionBox(gridObject.view.collisionData.boundingSquare) && (foundCollision = !0,
                ...
                foundCollision) {
                    ...
                    newPos.copy(oldPos),
                    newPos.y = y,
                    collisionGridObject instanceof GS.Monster && monster.scatter()
                } else
                    ;
            }
        }
    ...
    getSectorHeights: function(cells, box) {
        ...
        if (this.grid.forEachUniqueGridObjectInCells(cells, [GS.Concrete, GS.Elevator], function(gridObject) {
            ...
            if (GAME.grid.totalBoxSectorChecks++,
            GS.PolygonHelper.intersectionSectorBox(sector, box)) {
                var floorHeight = sector.floorTopY;
                floorHeight > result.floorHeight && (result.floorHeight = floorHeight,
                result.ceilHeight = sector.ceiling === !0 ? sector.ceilBottomY : 1 / 0),
                heights.push(floorHeight)
            }
        }),
        heights.length > 0) {
            heights.sort();
            for (var distance, maxDistance = 0, i = 0; i < heights.length - 1; i++)
    ...
    elevatorMove: function() {
        ...
        return function(elevator, velocity) {
            var types = [GS.Item];
            velocity > 0 && (types.push(GS.Monster),
            ...
            this.grid.forEachUniqueGridObjectInCells(elevator.linkedGridCells, types, function(gridObject) {
                if (!(gridObject instanceof GS.Item && gridObject.isStatic === !0) && (boundingBox = gridObject.view.collisionData.boundingBox,
                boundingBox.isIntersectionBox(elevatorBoundingBox))) {
                    if (gridObject instanceof GS.Item)
                        if (void 0 !== gridObject.elevatorId) {
                            if (gridObject.elevatorId != elevator.id)
                                return
                        } else {
                            if (gridObject.position.toVector2(pos2d),
                            !GS.PolygonHelper.sectorContainsPoint(elevator.sector, pos2d, !0))
                                return;
    ...
    collideProjectile: function() {
        ...
        return function(projectile, oldPos, newPos) {
            ...
            if (void 0 !== gridLocation) {
                ...
                result.type !== GS.CollisionTypes.None ? (result.type === GS.CollisionTypes.Environment ? (this.grid.addEnvironmentImpactParticles(result.pos, result.normal, projectile.color),
                result.gridObject.onHit()) : result.type === GS.CollisionTypes.Entity && (result.gridObject.constructor === projectile.sourceGridObject.constructor ? (normal.copy(result.pos).sub(projectile.position).normalize(),
                this.grid.addEnvironmentImpactParticles(result.pos, normal, projectile.color)) : (this.grid.addEntityImpactParticles(result.pos, result.gridObject.bloodColor),
                result.gridObject.onHit(projectile.damage))),
                projectile.updateCollisionData(result.pos),
                projectile.onHit()) : projectile.updateCollisionData(newPos),
                projectile.removed || this.updateGridLocationEllipsoid(projectile, newPos)
            }
        }
    }(),
    ...
    collideProjectileEntities: function() {
        ...
        return function(projectile, gridLocation, oldPos, newPos, result) {
            var gridObjectBox, projectileBox = projectile.view.collisionData.boundingBox, cells = this.grid.getCellsFromGridLocation(gridLocation);
            this.grid.forEachUniqueGridObjectInCells(cells, types, function(gridObject) {
                if (!gridObject.dead && projectile.sourceGridObject !== gridObject && (gridObjectBox = gridObject.view.collisionData.boundingBox,
                projectileBox.isIntersectionBoxSwept(oldPos, newPos, gridObjectBox, gridObject.position, collisionPoint))) {
    ...
    hitscan: function(sourceGridObject, projectileStart, weapon, xAngle, yAngle) {
        var that = this
          ...
          , typesEntity = [];
        sourceGridObject instanceof GS.Player && typesEntity.push(GS.Monster),
        GS.HitscanHelper.spread(weapon, xAngle, yAngle, function(dir) {
            ...
            result.type === GS.CollisionTypes.Environment ? (that.grid.addEnvironmentImpactParticles(result.pos, result.normal, weapon.impactParticleColor, weapon.impactParticleCount),
            result.gridObject.onHit()) : result.type === GS.CollisionTypes.Entity && (that.grid.addEntityImpactParticles(result.pos, result.gridObject.bloodColor),
...
GS.FPSControls.prototype = {
    ...
    attachEvents: function() {
        ...
        $(this.debugCanvas).on("mouseup.fpsControls", function(e) {
            3 == e.which && rightMouseDown && (rightMouseDown = !1,
            that.enable())
        })
    },
    ...
    enable: function() {
        this.pointerLockEnabled || (this.canvas.requestPointerLock = this.canvas.requestPointerLock || this.canvas.mozRequestPointerLock || this.canvas.webkitRequestPointerLock,
        this.canvas.requestPointerLock())
    },
    disable: function() {
        this.pointerLockEnabled && (document.exitPointerLock = document.exitPointerLock || document.mozExitPointerLock || document.webkitExitPointerLock,
        document.exitPointerLock())
    },
    onPointerLockChange: function(e) {
        ...
        isCanvas ? (this.pointerLockEnabled = !0,
        $(document).on("mousemove.fpsControls", function(e) {
            that.onMouseMove(e.originalEvent)
        }),
        this.dispatchEvent({
        ...
        $(document).off("mousemove.fpsControls"),
        void 0 !== this.disposeCallback ? (this.detachEvents(),
        this.disposeCallback()) : this.dispatchEvent({
    ...
    dispose: function(callback) {
        this.pointerLockEnabled && (this.disposeCallback = callback),
        this.onHandleCollisions = void 0,
        this.disable(),
        this.pointerLockEnabled || (this.detachEvents(),
        callback())
    }
},
...
GS.GraphicsManager.prototype = {
    ...
    initEffectComposer: function() {
        ...
        this.effectColor.uniforms.color.value = new THREE.Color(0),
    ...
    render: function() {
        ...
        if (this.renderer.autoClear = !1,
        this.renderer.clearTarget(renderTarget, !0, !0, !1),
        ...
        this.renderer.clearTarget(renderTarget, !1, !0, !1),
        this.showWeapon) {
            var playerView = this.grid.player.playerView;
    ...
    draw: function() {
        this.noPostProcessing ? this.renderToScreen() : (this.render(),
        this.ssaoEnabled && this.renderDepthTarget(),
        this.composer.render())
    },
    onResize: function() {
        var width = window.innerWidth
          , height = window.innerHeight;
        this._halfSizeEnabled && (width *= .5,
        height *= .5);
    ...
    getNoiseTexture: function(size) {
        ...
        return tex.wrapS = tex.wrapT = THREE.RepeatWrapping,
        tex.needsUpdate = !0,
        tex
    },
    reset: function() {
    ...
    set bloomEnabled(value) {
        void 0 !== this.effectBloom && (this.effectFilter.enabled = value,
...
GS.Grid = function(renderer, scene) {
    ...
    this.stopMonsters = !1,
    this.debugEntityMeshes = !1,
    this.debugConcreteMeshes = !1,
    this.debugCellLines = !1
}
,
GS.Grid.prototype = {
    constructor: GS.Grid,
    init: function() {
        ...
        this.debugCellLines && this.addDebugCellLines()
    },
    ...
    addEntityMeshesToScene: function() {
        ...
        this.forEachUniqueGridObject([GS.Item, GS.Monster, GS.Door, GS.Elevator, GS.TVScreen, GS.Switch], function(gridObject) {
            that.debugEntityMeshes && void 0 !== gridObject.view.debugMesh && gridObject.view.mesh.children.push(gridObject.view.debugMesh),
    ...
    addProjectile: function(sourceGridObject, type, position, direction) {
        ...
        projectile.init(),
        this.debugEntityMeshes && (projectile.view.debugMesh = new THREE.Mesh(new THREE.BoxGeometry(1,1,1),new THREE.MeshBasicMaterial({
            color: 16777215,
            wireframe: !0
        })),
        ...
        if (void 0 !== types && types.length > 0)
            for (var i = 0; i < cells.length; i++)
                for (var cell = cells[i], k = 0; k < cell.children.length; k++)
                    for (var gridObject = cell.children[k], e = 0; e < types.length; e++)
                        gridObject instanceof types[e] && (gridObjects[gridObject.id] = gridObject);
        else
            for (var i = 0; i < cells.length; i++)
                for (var cell = cells[i], k = 0; k < cell.children.length; k++) {
    ...
    getGridLocationFromPoints: function(points) {
        ...
        if (this.map.bounds.isIntersectionBox(box2)) {
            ...
            return gridMin.x = GS.MathHelper.clamp(gridMin.x, 0, this.width - 1),
    ...
    getGridCoords: function(point) {
        ...
        return p.x = Math.floor(p.x / this.cellSize),
    ...
    getTriangleIterator: function(gridLocation, types, condition) {
        ...
        return void 0 === condition ? function(callback) {
            that.forEachUniqueGridObjectInCells(cells, types, function(gridObject) {
                for (var triangles = gridObject.view.collisionData.triangles, i = 0; i < triangles.length; i += 3)
        ...
        return void 0 === condition ? function(callback) {
            that.forEachUniqueGridObjectInCells(cells, [GS.Concrete, GS.Elevator, GS.Door], function(gridObject) {
                if (gridObject instanceof GS.Concrete && gridObject.type === GS.MapLayers.Segment)
                    ...
                    for (var segs = gridObject.view.collisionData.segments, i = 0; i < segs.length; i++)
                        callback(segs[i])
            })
        }
        : function(callback) {
            that.forEachUniqueGridObjectInCells(cells, [GS.Concrete, GS.Elevator, GS.Door], function(gridObject) {
                if (gridObject instanceof GS.Concrete && gridObject.type === GS.MapLayers.Segment)
                    condition(gridObject.sourceObj) && callback(gridObject.sourceObj);
                else if (gridObject instanceof GS.Elevator || gridObject instanceof GS.Door)
                    for (var segs = gridObject.view.collisionData.segments, i = 0; i < segs.length; i++)
                        condition(segs[i]) && callback(segs[i])
            })
        }
    },
    clearScene: function() {
...
GS.MapScript.prototype = {
    ...
    getGridObjectsById: function(type, idArray) {
        ...
        for (var i in source)
            idArray.indexOf(parseInt(i, 10)) != -1 && list.push(source[i]);
        return list
    }
},
...
GS.MusicManager.prototype = {
    init: function() {
        ...
        if ("undefined" != typeof AudioContext)
            ...
            if ("undefined" == typeof webkitAudioContext)
                return void GAME.handleFatalError("No AudioContext support");
            ctx = new webkitAudioContext
        }
        this.ctx = ctx,
        this.ctx.createGain || (this.ctx.createGain = ctx.createGainNode),
    ...
    playTrack: function(name) {
        ...
        void 0 !== this.source && this.source.stop(0),
        this.source = this.ctx.createBufferSource(),
        this.source.buffer = this.currentTrack.buffer,
        this.source.loop = !0,
        this.source.connect(this.gainNode),
        this.source.start || (this.source.start = source.noteOn),
        this.source.start(0)
    },
    ...
    set volume(value) {
        this._volume = value,
        void 0 !== this.gainNode && (this.gainNode.gain.value = this._volume)
    },
    get volume() {
        return this._volume
    }
...
GS.AnimationView.prototype = {
    init: function() {
        var that = this;
        $.extend(!0, this, this.gridObject.view.mesh.userData),
        Object.keys(this.animations).forEach(function(key) {
            var loop = that.loops[key];
            return void 0 === that.loops[key] ? void GAME.handleFatalError("animation loop not found") : (loop.index = 0,
            loop.frames = that.animations[key],
            loop.max = loop.frames.length,
            void (loop.cooldown = Math.floor(Math.random() * (loop.delay - 1)) + 1))
        }),
        this.initInactiveLoop()
    },
    update: function() {
        var loop = this.currentLoop;
        loop.runOnce && loop.index === loop.max - 1 || loop.cooldown > 0 && (loop.cooldown--,
        0 === loop.cooldown && (loop.cooldown = loop.delay,
        loop.index++,
        loop.index === loop.max && (loop.index = 0),
        this.switchMesh())),
        this.gridObject.dead || (this.gridObject.moving ? this.floatAngle += this.floatSpeed : loop === this.loops.inactive && (this.floatAngle += this.inactiveFloatSpeed),
        this.positionYOffset = this.floatYDelta * Math.sin(this.floatAngle),
        Math.abs(this.painAngle) > 0 && (this.painAngle -= this.painSgn * this.painSpeed,
        1 == this.painSgn && this.painAngle < 0 ? (this.gridObject.inPain = !1,
        this.painAngle = 0) : this.painSgn == -1 && this.painAngle > 0 && (this.gridObject.inPain = !1,
        this.painAngle = 0)),
    ...
    pain: function() {
        0 === Math.floor(2 * Math.random()) ? (this.painAngle = this.painMaxAngle,
        this.painSgn = 1) : (this.painAngle = -this.painMaxAngle,
        this.painSgn = -1)
    },
    death: function() {
        this.setLoop("death"),
    ...
    switchMesh: function() {
        var loop = this.currentLoop;
        this.currentMesh.visible = !1,
        this.currentMesh = loop.frames[loop.index],
        this.currentMesh.visible = !0
    }
},
...
GS.LightingView.prototype = {
    ...
    updatePlayerLights: function() {
        ...
        this.worldLights.muzzleFlash.position.y += .5 * this.player.size.y,
    ...
    update: function() {
        for (var i = 0; i < this.projectileLights.length; i++) {
            ...
            light.active && (light.projectile.removed && light.intensity.setTargetValue(0),
            light.intensity.update(),
            light.pointLight.intensity = light.intensity.value,
            light.pointLight.position.copy(light.projectile.position),
            0 === light.intensity.value && (light.active = !1))
        }
    }
},
...
GS.ParticleView = function(grid) {
    ...
    this.geometries = {},
    this.materials = {},
    this.particleSpeed = 75e-5,
    this.particleGravity = new THREE.Vector3(0,(-.003),0)
}
,
GS.ParticleView.prototype = {
    ...
    update: function() {
        for (var i = 0; i < this.particles.length; i++) {
            ...
            particle.movementType != GS.ParticleMovementTypes.Static && (particle.landed !== !0 ? (particle.acceleration.add(particle.gravity),
            particle.velocity.add(particle.acceleration),
            particle.mesh.position.add(particle.velocity),
            particle.mesh.rotation.y += particle.rotationSpeed,
            particle.life++,
            void 0 !== particle.floorImpactY && particle.mesh.position.y < particle.floorImpactY && (particle.mesh.position.y = particle.floorImpactY + .5,
            particle.landed = !0)) : (particle.decayTime -= .01,
            particle.mesh.scale.set(particle.decayTime, particle.decayTime, particle.decayTime),
            particle.decayTime <= 0 && (particle.toBeRemoved = !0)))
        }
        for (var i = this.particles.length - 1; i >= 0; i--)
            this.particles[i].toBeRemoved && this.removeParticle(this.particles[i], i)
    },
    addParticle: function(width, height, depth, color, movementType, position) {
        ...
        return movementType != GS.ParticleMovementTypes.Static && (particle.gravity = this.particleGravity.clone(),
        movementType == GS.ParticleMovementTypes.Falling ? (particle.horizontalSpeed = 15,
        particle.verticalSpeed = 10 * Math.random()) : movementType == GS.ParticleMovementTypes.Gushing && (particle.horizontalSpeed = 20,
        particle.verticalSpeed = 10 * Math.random() + 60),
    ...
    removeParticle: function(particle, index) {
        ...
        i > -1 && (this.root.remove(particle.mesh),
    ...
    calculateFloorImpactY: function() {
        ...
        return function(particle) {
            ...
            if (void 0 !== gridLocation) {
                ...
                void 0 !== sector && (particle.floorImpactY = sector.floorTopY),
    ...
    calculateAcceleration: function(particle) {
        particle.acceleration = new THREE.Vector3(2 * Math.random() - 1,0,2 * Math.random() - 1),
    ...
    alignParticleTrajectoryToNormal: function() {
        ...
        return function(particle, normal) {
            if (0 === normal.x && 0 === normal.z) {
                if (normal.y == -1)
                    return;
                return particle.horizontalSpeed = 20,
                particle.verticalSpeed = 10 * Math.random() + 30,
                void this.calculateAcceleration(particle)
            }
    ...
    getGeometry: function(width, height, depth) {
        ...
        return id in this.geometries || (this.geometries[id] = new THREE.BoxGeometry(width,height,depth)),
        this.geometries[id]
    },
...
GS.PlayerView = function() {
    ...
    $.extend(!0, this.weapons, GS.Weapons),
    this.scene = new THREE.Scene,
    this.camera = new THREE.PerspectiveCamera(75,window.innerWidth / window.innerHeight,.1,100),
    this.screenOverlayColor = new THREE.Color(0),
    this.screenOverlayColorFadeSpeed = .02,
    ...
    this.viewThrottle = new GS.SmoothNumber(0,.1),
    this.weaponThrottle = new GS.SmoothNumber(0,.1),
    this.tweens = {
        shoot: {
...
GS.PlayerView.prototype = {
    ...
    initWeaponMesh: function(name) {
        ...
        void 0 !== weapon.mesh && (weapon.mesh.visible = !1,
    ...
    updateMuzzleFlash: function() {
        this.muzzleCooldown > 0 && (this.muzzleCooldown--,
        0 === this.muzzleCooldown && this.lightingView.endMuzzleFlash())
    },
    updateLightLevel: function() {
    ...
    updateThrottle: function(oldPos, newPos) {
        ...
        !this.player.shooting && this.weaponReady && (this.weaponThrottle.setTargetValue(velocity / this.player.moveSpeed),
    ...
    updateViewBob: function() {
        this.viewBob.enabled && (this.player.controls.setViewOffsetY(this.viewBob.offset),
    ...
    updateWeaponBob: function() {
        if (this.weaponBob.enabled && !this.player.shooting) {
            ...
            void 0 !== this.weapon && this.weapon.mesh.position.copy(this.weapon.position.clone().add(this.weaponBob.offset))
        }
    },
    shoot: function() {
    ...
    changeToWeaponInstant: function(name) {
        this.resetWeaponThrottle(),
        void 0 !== this.weapon && (this.weapon.mesh.visible = !1),
        this.weaponReady = !0,
        this.weapon = this.weapons[name],
        this.weapon.mesh.visible = !0,
    ...
    changeToWeapon: function(name) {
        ...
        this.resetWeaponThrottle(),
        this.weaponReady = !1,
        void 0 !== this.weapon ? this.hideWeapon(function() {
    ...
    onItemPickup: function() {
        this.screenOverlayColor.setRGB(.3, .3, 0),
        this.dispatchEvent({
    ...
    onDamage: function() {
        this.screenOverlayColor.setRGB(.6, 0, 0),
        this.dispatchEvent({
            type: "screenOverlayColorChange",
            color: this.screenOverlayColor
        })
    ...
    onDeath: function() {
        ...
        void 0 !== this.weapon && this.hideWeapon()
    },
    updateScreenOverlay: function() {
        (this.screenOverlayColor.r > 0 || this.screenOverlayColor.g > 0 || this.screenOverlayColor.b > 0) && (this.screenOverlayColor.addScalar(-this.screenOverlayColorFadeSpeed),
        this.screenOverlayColor.r < 0 && (this.screenOverlayColor.r = 0),
        this.screenOverlayColor.g < 0 && (this.screenOverlayColor.g = 0),
        this.screenOverlayColor.b < 0 && (this.screenOverlayColor.b = 0),
        this.dispatchEvent({
            type: "screenOverlayColorChange",
            color: this.screenOverlayColor
        }))
...
GS.Game.prototype = GS.inherit(GS.Base, {
    ...
    preInit: function() {
        ...
        this.firstPlay = !0,
        this.mapWon = !1,
        this.restartedLevel = !1,
        this.antialias = !1,
        this.clearColor = 3368601,
        this.cameraFov = GS.Settings.fov,
        this.cameraFar = 1500,
        this.isTestMap() ? this.noMenu = !0 : this.noMenu = !1,
        this.firstTimeInMenu = !0,
        this.showFPS = GS.Settings.showFPS,
        this.showPerformanceDebugMeters = !1
    },
    init: function() {
        GS.DebugUI.init(),
        GS.DebugUI.visible = !1,
    ...
    preLoad: function() {
        ...
        this.assetLoader.loaded ? this.nextState = GS.GameStates.PostLoad : (this.assetLoader.load(),
    ...
    postLoad: function() {
        this.firstLoad ? (this.loadingUI.spinnerOnly = !0,
        this.uiManager.initComponents(this.assetLoader.assets),
        this.openMenu(),
        this.firstLoad = !1,
        this.noMenu && this.newGame()) : (this.initComponents(this.assetLoader.assets),
        ...
        void 0 === this.grid.aiManager.script || this.restartedLevel || this.musicManager.playTrack(this.grid.aiManager.script.musicTrack),
        this.restartedLevel = !1,
        this.firstPlay && (this.firstPlay = !1))
    },
    play: function() {
        GS.InputHelper.checkPressedKeys(),
        !GS.InputHelper.keysPressed && GS.InputHelper.isKeyDown(this.keys.Escape) && this.openMenu(),
        this.grid.aiManager.mapWon || GS.InputHelper.keysPressed || !GS.InputHelper.isKeyDown(this.keys.Tab) || (this.uiManager.automap.visible = !this.uiManager.automap.visible,
        this.uiManager.overrideRedraw = !0,
        this.graphicsManager.monochromeEnabled = this.uiManager.automap.visible),
        this.grid.player.dead && !GS.InputHelper.keysPressed && GS.InputHelper.isKeyDown(this.keys.Enter) && this.restartLevel(),
        this.grid.aiManager.mapWon && !GS.InputHelper.keysPressed && GS.InputHelper.isKeyDown(this.keys.Enter) && void 0 !== this.grid.aiManager.script.nextMap && (this.playerPersistencePackage = this.grid.player.getPersistencePackage(),
        this.loadLevel(this.grid.aiManager.script.nextMap)),
        !this.mapWon && this.grid.aiManager.mapWon && (this.mapWon = !0,
        this.onMapWon()),
        this.grid.aiManager.mapWon || (this.grid.update(),
        TWEEN.update()),
        GS.DebugUI.update(),
    ...
    menu: function() {
        GS.InputHelper.checkPressedKeys(),
        !GS.InputHelper.keysPressed && GS.InputHelper.isKeyDown(this.keys.Escape) && void 0 !== this.grid && this.closeMenu(),
        this.uiManager.update()
    },
    openMenu: function() {
        this.nextState = GS.GameStates.Menu,
        void 0 !== this.grid && (this.grid.player.inMenu = !0,
        this.grid.player.controls.disable(),
        this.grid.player.controls.detachEvents()),
        this.graphicsManager.monochromeEnabled = !0,
        this.uiManager.menuActive = !0,
        GS.DebugUI.visible = !1
    },
    closeMenu: function() {
        this.nextState = GS.GameStates.Play,
        void 0 !== this.grid ? (this.grid.player.inMenu = !1,
        this.grid.player.controls.attachEvents(),
        this.grid.player.controls.enable(),
        this.grid.aiManager.resume(),
        !this.mapWon && this.uiManager.automap.visible || (this.graphicsManager.monochromeEnabled = !1)) : this.graphicsManager.monochromeEnabled = !1,
        this.firstTimeInMenu && (this.uiManager.removeNewsBox(),
        this.firstTimeInMenu = !1),
        this.uiManager.menuActive = !1,
        GS.DebugUI.visible = GS.Settings.showHUD
    },
    restartLevel: function() {
        this.nextState = GS.GameStates.Dispose,
        this.restartedLevel = !0
    },
    loadLevel: function(name) {
        this.uiManager.menuActive && this.closeMenu(),
        this.mapName = name,
        this.nextState = GS.GameStates.Dispose
    },
    newGame: function() {
        this.uiManager.menuActive && this.closeMenu(),
        this.isTestMap() ? this.mapName = "testMap" : this.mapName = "airstrip1",
        this.nextState = GS.GameStates.Dispose
    },
    isTestMap: function() {
    ...
    initComponents: function(assets) {
        ...
        void 0 !== this.playerPersistencePackage && (this.grid.player.applyPersistencePackage(this.playerPersistencePackage),
        this.playerPersistencePackage = void 0),
        this.grid.update(),
        this.graphicsManager.setGrid(this.grid),
        this.grid.player.controls.addEventListener("pointerLockDisabled", function() {
            that.openMenu()
        })
    },
    update: function() {
        var time;
        this.showPerformanceDebugMeters && (time = window.performance.now()),
        this.state == GS.GameStates.Dispose ? this.updated || (this.updated = !0,
        this.dispose()) : this.state == GS.GameStates.PreLoad ? this.updated || (this.updated = !0,
        this.preLoad()) : this.state == GS.GameStates.Loading ? this.updated = !0 : this.state == GS.GameStates.PostLoad ? this.updated || (this.updated = !0,
        this.postLoad()) : this.state == GS.GameStates.Play ? (this.updated = !0,
        this.play()) : this.state == GS.GameStates.Menu && (this.updated = !0,
        this.menu()),
        null !== this.nextState && this.updated && (this.nextState != GS.GameStates.Play && this.nextState != GS.GameStates.Menu || (this.loadingUI.hide(),
        this.uiManager.show()),
        this.state = this.nextState,
        this.nextState = null,
        this.updated = !1),
        this.showPerformanceDebugMeters && (this.updateTime = (window.performance.now() - time).toFixed(2),
        GS.DebugUI.trackNumericValue("updateTime", this.updateTime))
    },
    draw: function() {
        var time;
        this.showPerformanceDebugMeters && (time = window.performance.now()),
        this.state == GS.GameStates.PreLoad ? this.loadingUI.draw() : this.state == GS.GameStates.Loading ? this.loadingUI.draw() : this.state == GS.GameStates.PostLoad ? this.loadingUI.draw() : this.state != GS.GameStates.Play && this.state != GS.GameStates.Menu || (void 0 !== this.grid && this.graphicsManager.draw(),
        this.uiManager.draw()),
        this.showPerformanceDebugMeters && (this.drawTime = (window.performance.now() - time).toFixed(2),
    ...
    onResize: function() {
        ...
        void 0 !== this.graphicsManager && this.graphicsManager.onResize(),
        void 0 !== this.grid && (this.grid.onResize(),
    ...
    customMap: function() {
        ...
        if (void 0 === files || 0 === files.length)
            return void customMapInput.trigger("click");
        var file = files[0]
    ...
    updateFov: function() {
        ...
        this.camera.updateProjectionMatrix(),
        this.state == GS.GameStates.Play && this.grid.updateFov()
    },
    disposeEnd: function() {
        TWEEN.removeAll(),
        this.grid = void 0,
...
GS.Settings = function() {
    ...
    return {
        loadSettings: function() {
            ...
            if (void 0 !== jsonStr) {
                ...
                for (var i in loadedSettings)
                    i in settings && (settings[i] = loadedSettings[i]);
        ...
        set fov(value) {
            ...
            isNaN(n) || (n = Math.floor(GS.MathHelper.clamp(n, settings.fovMin, settings.fovMax)),
            settings.fov = n,
            GAME.updateFov(),
            this.saveSettings())
        },
        ...
        set viewBob(value) {
            settings.viewBob = value === !0,
            this.saveSettings(),
            void 0 !== GAME.grid && (GAME.grid.player.playerView.viewBob.enabled = settings.viewBob)
        },
        get viewBob() {
            return settings.viewBob
        },
        set weaponBob(value) {
            settings.weaponBob = value === !0,
            this.saveSettings(),
            void 0 !== GAME.grid && (GAME.grid.player.playerView.weaponBob.enabled = settings.weaponBob)
        },
        get weaponBob() {
            return settings.weaponBob
        },
        set halfSize(value) {
            settings.halfSize = value === !0,
            this.saveSettings(),
        ...
        set sound(value) {
            var n = parseInt(value);
            isNaN(n) || (n = Math.floor(GS.MathHelper.clamp(n, settings.soundMin, settings.soundMax)),
            settings.sound = n,
        ...
        set music(value) {
            var n = parseInt(value);
            isNaN(n) || (n = Math.floor(GS.MathHelper.clamp(n, settings.musicMin, settings.musicMax)),
            settings.music = n,
        ...
        set mouse(value) {
            var n = parseInt(value);
            isNaN(n) || (n = Math.floor(GS.MathHelper.clamp(n, settings.mouseMin, settings.mouseMax)),
            settings.mouse = n,
            void 0 !== GAME.grid && (GAME.grid.player.controls.lookSpeed = .066 * (settings.mouse / 5)),
            this.saveSettings())
        },
        get mouse() {
        ...
        set showHUD(value) {
            settings.showHUD = value === !0,
            this.saveSettings(),
            GAME.uiManager.showHUD = settings.showHUD,
            GAME.uiManager.overrideRedraw = !0
        },
        get showHUD() {
            return settings.showHUD
        },
        set showWeapon(value) {
            settings.showWeapon = value === !0,
            this.saveSettings(),
        ...
        set mouseInvertY(value) {
            settings.mouseInvertY = value === !0,
            this.saveSettings(),
            void 0 !== GAME.grid && (GAME.grid.player.controls.mouseInvertY = settings.mouseInvertY)
        },
        get mouseInvertY() {
